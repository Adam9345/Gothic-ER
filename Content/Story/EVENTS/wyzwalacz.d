...
func void WYZWALACZ ()
{
 ...
 {
 loc_OC = true;
 PrintS_Ext ("Nowa lokacja szybkiej podró¿y: Stary Obóz!",COL_White) ;
 };
 
 if (loc_ricefields == false) && (Npc_GetDistToWP (hero, "NC_PATH55") < 1000)
 {
 loc_ricefields = true;
 PrintS_Ext ("Nowa lokacja szybkiej podró¿y: Pola ry¿owe!",COL_White) ;
 };
 
 if (loc_NC_cave == false) && (Npc_GetDistToWP (hero, "NC_PATH_TO_PIT_04") < 1000)
 {
 loc_NC_cave = true;
 PrintS_Ext ("Nowa lokacja szybkiej podró¿y: Nowy Obóz - jaskinia!",COL_White) ;
 };
 
 if (loc_FMC == false) && (Npc_GetDistToWP (hero, "FMC_PATH06") < 1000)
 {
 loc_FMC = true;
 PrintS_Ext ("Nowa lokacja szybkiej podró¿y: Kocio³!",COL_White) ;
 };
 
  if (loc_OMC == false) && (Npc_GetDistToWP (hero, "OW_OM_ENTRANCE01") < 1000)
 {
 loc_OMC = true;
 PrintS_Ext ("Nowa lokacja szybkiej podró¿y: Obóz przy Starej Kopalni!",COL_White) ;
 };
 
 if (loc_QC == false) && (Npc_GetDistToWP (hero, "LOCATION_11_06") < 1000)
 {
 loc_QC = true;
 PrintS_Ext ("Nowa lokacja szybkiej podró¿y: Obóz Bandytów!",COL_White) ;
 };
 
 if (loc_hunters == false) && (Npc_GetDistToWP (hero, "SPAWN_TALL_PATH_BANDITOS2") < 1000)
 {
 loc_hunters = true;
 PrintS_Ext ("Nowa lokacja szybkiej podró¿y: Obóz myœliwych!",COL_White) ;
 };
 
 if (loc_abmine == false) && (Npc_GetDistToWP (hero, "OW_PATH_1_17") < 1000)
 {
 loc_abmine = true;
 PrintS_Ext ("Nowa lokacja szybkiej podró¿y: Opuszczona kopalnia!",COL_White) ;
 };
 
 if (loc_orchunters == false) && (Npc_GetDistToWP (hero, "HUNTERS_LADDERR2") < 1000)
 {
 loc_orchunters = true;
 PrintS_Ext ("Nowa lokacja szybkiej podró¿y: £owcy Orków!",COL_White) ;
 };
 
 if (loc_citadel == false) && (Npc_GetDistToWP (hero, "CASTLE_19") < 1000)
 {
 loc_citadel = true;
 PrintS_Ext ("Nowa lokacja szybkiej podró¿y: Cytadela!",COL_White) ;
 };
 
  if (loc_cavalorn == false) && (Npc_GetDistToWP (hero, "PATH_OC_NC_4") < 1000)
 {
 loc_cavalorn = true;
 PrintS_Ext ("Nowa lokacja szybkiej podró¿y: Chata Cavalorna!",COL_White) ;
 };
 
   if (loc_aidan == false) && (Npc_GetDistToWP (hero, "PATH_OC_NC_14") < 1000)
 {
 loc_aidan = true;
 PrintS_Ext ("Nowa lokacja szybkiej podró¿y: Chata Aidana!",COL_White) ;
 };
 
    if (loc_klasztor == false) && (Npc_GetDistToWP (hero, "OW_PATH_BRIDGE_01") < 1000)
 {
 loc_klasztor = true;
 PrintS_Ext ("Nowa lokacja szybkiej podró¿y: Most przy klasztorze!",COL_White) ;
 };
 
     if (loc_fogtower == false) && (Npc_GetDistToWP (hero, "OW_PATH_174") < 1000)
 {
 loc_fogtower = true;
 PrintS_Ext ("Nowa lokacja szybkiej podró¿y: Wie¿a Mgie³!",COL_White) ;
 };
 ...
 {
 loc_swamp = true;
 PrintS_Ext ("Nowa lokacja szybkiej podró¿y: Bagno!",COL_White) ;
 };
 
  if (loc_forest == false) && (Npc_GetDistToWP (hero, "OW_PATH_292") < 1000)
 {
 loc_forest = true;
 PrintS_Ext ("Nowa lokacja szybkiej podró¿y: Wielki Las!",COL_White) ;
 };
 
   if (loc_orcdesert == false) && (Npc_GetDistToWP (hero, "OW_ORC_LOOKOUT_2_02_MOVE2") < 1000)
 {
 loc_orcdesert = true;
 PrintS_Ext ("Nowa lokacja szybkiej podró¿y: Ziemie Orków!",COL_White) ;
 };
...
{
    VictorZL_Dead = true;
	B_LogEntry               (KsiegaDlaVictora,"Zabi³em Victora i zakoñczy³em jego chore obrzêdy. Chyba wiem co sta³o siê z moimi poprzednikami. Dziennik Victora to ostateczny dowód jego zbrodni. Pora udaæ siê do Quentina i wyjaœniæ zagadkê znikaj¹cych ludzi.");
};	
