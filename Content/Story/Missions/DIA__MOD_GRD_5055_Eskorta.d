...
INSTANCE DIA_Eskorta_FOREST (C_INFO)
{
   npc          = GRD_5055_Eskorta;
   nr           = 1;
   condition    = DIA_Eskorta_FOREST_Condition;
   information  = DIA_Eskorta_FOREST_Info;
   permanent	= FALSE;
   Important    = TRUE;
};

FUNC INT DIA_Eskorta_FOREST_Condition()
{

if (Npc_GetDistToWP(hero,"OW_PATH_1_1")<4000)
{
    return TRUE;
};
};
FUNC VOID DIA_Eskorta_FOREST_Info()
{

    AI_GotoNpc			(self,	hero);
    AI_Output (self, other ,"DIA_Eskorta_FOREST_03_01"); //Powinniœmy przejœæ las kanionem po jego œrodku.
    AI_Output (self, other ,"DIA_Eskorta_FOREST_03_02"); //Tak bêdzie najszybciej i najsprawniej.
    AI_Output (other, self ,"DIA_Eskorta_FOREST_15_03"); //To œwietne miejsce na potencjaln¹ zasadzkê.
    AI_Output (self, other ,"DIA_Eskorta_FOREST_03_04"); //W tym lesie ka¿de miejsce jest zdradliwe.
    AI_Output (self, other ,"DIA_Eskorta_FOREST_03_05"); //Rób co mówiê. To nie przelewki.
    AI_Output (other, self ,"DIA_Eskorta_FOREST_15_06"); //Zrobiê to co uwa¿am za s³uszne w tej sytuacji. 
};
