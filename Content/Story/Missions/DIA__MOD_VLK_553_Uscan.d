...
FUNC VOID DIA_Uscan_Flour_Info()
{
	...
	B_GiveInvItems (other,self, ItMi_FlourBag, 5);
...
INSTANCE DIA_Uscan_Mine (C_INFO)
{
   npc          = VLK_553_Buddler;
   nr           = 1;
   condition    = DIA_Uscan_Mine_Condition;
   information  = DIA_Uscan_Mine_Info;
   permanent	= FALSE;
   description	= "Czeœæ. Przysy³a mnie Lares.";
};

FUNC INT DIA_Uscan_Mine_Condition()
{
    if (Npc_KnowsInfo (hero, DIA_Mordrag_Mine))
    {
    return TRUE;
    };
};
...
FUNC VOID DIA_Uscan_Mine_Info()
{
    AI_Output (other, self ,"DIA_Uscan_Mine_15_01"); //Czeœæ. Przysy³a mnie Lares.
    AI_Output (self, other ,"DIA_Uscan_Mine_03_02"); //Nie znam ¿adnego Laresa.
    AI_Output (other, self ,"DIA_Uscan_Mine_15_03"); //(Szeptem) Œmieræ Magnatom.
    AI_Output (self, other ,"DIA_Uscan_Mine_03_04"); //Aaa, to zmienia sytuacje. O co chodzi?
    AI_Output (other, self ,"DIA_Uscan_Mine_03_05"); //Podobno z Obozu niebawem wyruszyæ konwój, który ma chroniæ in¿yniera, który ma poczyniæ niezbêdne naprawy w Starej Kopalni
    AI_Output (self, other ,"DIA_Uscan_Mine_03_06"); //To prawda.
    AI_Output (other, self ,"DIA_Uscan_Mine_15_07"); //Powiedz kiedy dok³adniej to bêdzie.
	AI_Output (self, other ,"DIA_Uscan_Mine_03_08"); //Najpierw moje wynagrodzenie. Informacje kosztuj¹. Daj mi 200 bry³ek rudy.
	AI_Output (other, self ,"DIA_Uscan_Mine_15_09"); //Ile?!
    AI_Output (self, other ,"DIA_Uscan_Mine_03_10"); //Nie przes³ysza³eœ siê. I nie próbuj mnie straszyæ.

    B_LogEntry                     (Zawirowania,"Uscan to nasz informator w starym obozie. Mam mu zap³acic 200 bry³ek rudy ¿eby przekaza³ mi informacje na temat konwoju który wkrótce ma wyruszyæ w stronê starej kopalni.");
...
INSTANCE DIA_Uscan_Mine1 (C_INFO)
{
   npc          = VLK_553_Buddler;
   nr           = 1;
   condition    = DIA_Uscan_Mine1_Condition;
   information  = DIA_Uscan_Mine1_Info;
   permanent	= FALSE;
   description	= "Proszê w takim razie weŸ to.";
};

FUNC INT DIA_Uscan_Mine1_Condition()
{
    if (Npc_KnowsInfo (hero, DIA_Uscan_Mine))
...
FUNC VOID DIA_Uscan_Mine1_Info()
{
    AI_Output (other, self ,"DIA_Uscan_Mine1_15_01"); //Proszê w takim razie weŸ to.
    AI_Output (self, other ,"DIA_Uscan_Mine1_03_02"); //Wiêc konwój z in¿ynierem wyrusza ju¿ jutro.
	B_GiveInvItems (other,self, ItMiNugget, 200);
    AI_Output (other, self ,"DIA_Uscan_Mine1_15_03"); //Dziêki za t¹ wiadomoœæ, trzymaj siê.
  
   

    B_LogEntry (Zawirowania,"Uscan powiedzia³ za op³at¹, ¿e konwój z in¿ynierem wyruszy ju¿ jutro. Muszê siê poœpieszyæ i jak najszybciej donieœæ o tym Laresowi.");

    B_GiveXP (50);

};

instance dia_vlk_553_pickpocket(c_info) {
    npc = vlk_553_buddler;
    nr = 900;
    condition = dia_vlk_553_pickpocket_condition;
    information = dia_vlk_553_pickpocket_info;
    permanent = 1;
    description = pickpocket_final;
};

func int dia_vlk_553_pickpocket_condition() {
	e_beklauen(baseThfChanceVLK, 20);
};

func void dia_vlk_553_pickpocket_info() {
	b_steal_message();
	info_clearchoices(dia_vlk_553_pickpocket);
	info_addchoice(dia_vlk_553_pickpocket, dialog_back, dia_vlk_553_pickpocket_back);
	info_addchoice(dia_vlk_553_pickpocket, dialog_pickpocket, dia_vlk_553_pickpocket_doit);
};

func void dia_vlk_553_pickpocket_doit() {
    d_beklauen();
    info_clearchoices(dia_vlk_553_pickpocket);
};

func void dia_vlk_553_pickpocket_back() {
    info_clearchoices(dia_vlk_553_pickpocket);
};
