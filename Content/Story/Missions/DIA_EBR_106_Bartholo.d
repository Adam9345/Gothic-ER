...
FUNC VOID DIA_Bartholo_WTF2_HELP()
{
    ...
    AI_Output (self, other ,"DIA_Bartholo_WTF2_HELP_03_07"); //Uparty jesteœ... by³o tam kilka pucharów, amulet, ruda i pewien cenny pierœcieñ. Zale¿y mi g³ównie na pierœcieniu. 
...
INSTANCE DIA_Bartholo_PNA_Nagroda (C_INFO)
{
   npc          = EBR_106_Bartholo;
   nr           = 2;
   condition    = DIA_Bartholo_PNA_Nagroda_Condition;
   information  = DIA_Bartholo_PNA_Nagroda_Info;
   permanent	= FALSE;
   description	= "Chcia³eœ siê ze mn¹ widzieæ.";
};

FUNC INT DIA_Bartholo_PNA_Nagroda_Condition()
{
    if (Npc_KnowsInfo (hero, DIA_Fisk_PNA_Skory))
...
FUNC VOID DIA_Bartholo_PNA_Nagroda_Info()
{
    AI_Output (other, self ,"DIA_Bartholo_PNA_Nagroda_15_01"); //Chcia³eœ siê ze mn¹ widzieæ.
    AI_Output (self, other ,"DIA_Bartholo_PNA_Nagroda_03_02"); //Czy umówiona iloœæ skór dotar³a do handlarza Fiska?
    AI_Output (other, self ,"DIA_Bartholo_PNA_Nagroda_15_03"); //Tak, dotar³a.
    AI_Output (self, other ,"DIA_Bartholo_PNA_Nagroda_03_04"); //Zatem zas³u¿yliœcie na odpowiednie wynagrodzenie.
	B_GiveInvItems (self,other, itminugget, 400);
    AI_Output (other, self ,"DIA_Bartholo_PNA_Nagroda_15_05"); //Z grzecznoœci nie zaprzeczê.
    AI_Output (self, other ,"DIA_Bartholo_PNA_Nagroda_03_06"); //Oto wasza nagroda.
	AI_Output (other, self ,"DIA_Bartholo_PNA_Nagroda_15_07"); //Dziêkuje. To by³o przyjemne zlecenie. Polecamy siê na przysz³oœæ.
	AI_Output (self, other ,"DIA_Bartholo_PNA_Nagroda_03_08"); //Przyjemne zlecenie, przyjemna zap³ata. Bêdziemy o tym pamiêtaæ.
	AI_Output (other, self ,"DIA_Bartholo_PNA_Nagroda_15_09"); //Do zobaczenia.
    AI_Output (self, other ,"DIA_Bartholo_PNA_Nagroda_03_10"); //Trzymaj siê.
	Npc_ExchangeRoutine(EBR_106_Bartholo,"start");
	 B_LogEntry               (PolowanieNaAligatory,"Bartholo by³ zadowolony z naszej transakcji. Wyp³aci³ mi sowit¹ sumê 400 bry³ek za skóry aligatorów. Pora wywi¹zaæ siê z umowy i udaæ siê do obozu Alexa by siê rozliczyæ.");
	
};

instance dia_bartholo_pickpocket(c_info) {
    npc = ebr_106_bartholo;
    nr = 900;
    condition = dia_bartholo_pickpocket_condition;
    information = dia_bartholo_pickpocket_info;
    permanent = 1;
    description = pickpocket_final;
};

func int dia_bartholo_pickpocket_condition() {
	e_beklauen(baseThfChanceEBR, 40);
};

func void dia_bartholo_pickpocket_info() {
	b_steal_message();
	info_clearchoices(dia_bartholo_pickpocket);
	info_addchoice(dia_bartholo_pickpocket, dialog_back, dia_bartholo_pickpocket_back);
	info_addchoice(dia_bartholo_pickpocket, dialog_pickpocket, dia_bartholo_pickpocket_doit);
};

func void dia_bartholo_pickpocket_doit() {
    d_beklauen();
    info_clearchoices(dia_bartholo_pickpocket);
};

func void dia_bartholo_pickpocket_back() {
    info_clearchoices(dia_bartholo_pickpocket);
};
