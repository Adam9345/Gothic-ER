...
{
if (ConnorRemoveWeapon == TRUE)
{
 AI_RemoveWeapon(BAN_40028_Connor);
};
...
INSTANCE DIA_Talas_KH_Tirutt (C_INFO)
{
   npc          = Nov_1300_Talas;
   nr           = 1;
   condition    = DIA_Talas_KH_Tirutt_Condition;
   information  = DIA_Talas_KH_Tirutt_Info;
   permanent	= FALSE;
   description	= "Zna³eœ Tirutta?";
};

FUNC INT DIA_Talas_KH_Tirutt_Condition()
{
if (Npc_KnowsInfo(hero,DIA_BaalCadar_KH_Info))
{
    return TRUE;
};
};
FUNC VOID DIA_Talas_KH_Tirutt_Info()
{
    AI_Output (other, self ,"DIA_Talas_KH_Tirutt_15_01"); //Zna³eœ Tirutta?
    AI_Output (self, other ,"DIA_Talas_KH_Tirutt_03_02"); //To by³ mój dobry kumpel.
    ...
    AI_Output (other, self ,"DIA_Talas_KH_Tirutt_15_10"); //Gdybym zapewni³ ci eskortê i uprzednio zap³aci³ to wskaza³byœ mi to miejsce?
    AI_Output (self, other ,"DIA_Talas_KH_Tirutt_03_11"); //Eee... Wola³bym chyba zostaæ... Tyle, ¿e wiem jak tam siê idzie a nie, w  którym miejscu na mapie siê to znajduje. Jak g³upio by to nie zabrzmia³o.
	B_LogEntry               (KoronaHrodgira,"Talas wie gdzie Tirutt ukry³ koronê. Lecz nie jest chêtny by wskazaæ drogê do artefaktu, muszê go jakoœ przekonaæ. Wpierw jednak pójdê po Connora i zbiorê te cholerne 200 bry³ek dla Talasa...");
};

//========================================
//-----------------> KH_Path
//========================================
var int ConnorRemoveWeapon;
Func void ConnorAngry()
{

    B_SmartTurnToNpc(BAN_40028_Connor, Nov_1300_Talas);         
	AI_ReadyMeleeWeapon		(BAN_40028_Connor);
	AI_PlayAni				(BAN_40028_Connor, "T_1HSINSPECT");
	AI_RemoveWeapon			(BAN_40028_Connor);
	AI_RemoveWeapon			(BAN_40028_Connor);
	//AI_UnequipWeapons		(BAN_40028_Connor);
	ConnorRemoveWeapon = TRUE;
	return;

};
INSTANCE DIA_Talas_KH_Path (C_INFO)
{
   npc          = Nov_1300_Talas;
   nr           = 2;
   condition    = DIA_Talas_KH_Path_Condition;
   information  = DIA_Talas_KH_Path_Info;
   permanent	= FALSE;
   description	= "Czyli pójdziesz ze mn¹.";
};

FUNC INT DIA_Talas_KH_Path_Condition()
{
if (Npc_KnowsInfo(hero,DIA_Connor_KH_TALAS_SSIE))
&& (Npc_HasItems (other, ItMiNugget) >=200)
{
    return TRUE;
};
};
FUNC VOID DIA_Talas_KH_Path_Info()
{
    AI_Output (other, self ,"DIA_Talas_KH_Path_15_01"); //Czyli pójdziesz ze mn¹.
    ...
    AI_Output (self, other ,"DIA_Talas_KH_Path_03_08"); //A gdzie moja ruda?
    AI_Output (other, self ,"DIA_Talas_KH_Path_15_09"); //Trzymaj...
	B_GiveInvItems (other,self, itminugget, 200);
    AI_Output (self, other ,"DIA_Talas_KH_Path_03_10"); //Có¿... No to w drogê... 
	Npc_ExchangeRoutine(self,"KHPTH");
	Nov_1300_Talas.aivar[AIV_PARTYMEMBER] = TRUE;
	Wld_InsertNpc		(Snapper_Kh,"SPAWN_OW_BLOODHOUND_01");
	B_LogEntry               (KoronaHrodgira,"Zap³aci³em rud¹ Talasowi. Ma nas teraz zaprowadziæ na miejsce ukrycia korony. Tyle, ¿e ponoæ grasuje tam olbrzymi zêbacz. Damy sobie z nim radê.");
    AI_StopProcessInfos	(self); 
};
...
Func Void TalasRun()
{
			    ...
			    Nov_1300_Talas.aivar[AIV_PARTYMEMBER] = FALSE;
	           // Mdl_ApplyOverlayMds	(self,	"HUMANS_FLEE.MDS");
};
INSTANCE DIA_Talas_KH_NAURA (C_INFO)
{
   npc          = Nov_1300_Talas;
   nr           = 1;
   condition    = DIA_Talas_KH_NAURA_Condition;
   information  = DIA_Talas_KH_NAURA_Info;
   permanent	= FALSE;
   Important    = TRUE;
};

FUNC INT DIA_Talas_KH_NAURA_Condition()
{
if (Npc_KnowsInfo(hero,DIA_Talas_KH_Path))
&& (Npc_GetDistToWP(hero,"WP_KH_ADD")<1000)
{
    return TRUE;
};
};
FUNC VOID DIA_Talas_KH_NAURA_Info()
{
    AI_Output (self, other ,"DIA_Talas_KH_NAURA_03_01"); //Dobra ja wracam do obozu. Pozdrówcie ode mnie Beliara!
    AI_Output (other, self ,"DIA_Talas_KH_NAURA_15_02"); //Co za ³ajza...

	Npc_ExchangeRoutine(Nov_1300_Talas,"Start");
	B_LogEntry               (KoronaHrodgira,"Talas zwia³ a ja Connor musimy siê zaj¹æ wielkim zêbaczem. To nie bêdzie ³atwa walka.");
	TalasRun ();
	AI_StopProcessInfos	(self);
