...
{
	if	((Npc_GetTrueGuild(hero) == GIL_NONE) || (Npc_GetTrueGuild(hero) == GIL_SFB ))
	&&	(Kapitel < 2)
	{
		return 1;
	};
...
FUNC VOID  ORG_801_Lares_Reicht_Info()
{
	...
	if ( (Points_NC >= 29) && (hero.level >= 5) )
	{
		...
		B_LogEntry				(CH1_EasyJoinOC,	"Nic nie wysz³o z mojego u³atwionego przyjêcia do Starego Obozu. Zosta³em cz³onkiem bandy Laresa.");
...
INSTANCE ORG_801_Lares_Kopalnia (C_INFO)
{
	npc			= ORG_801_Lares;
	nr			= 1;
	condition	= ORG_801_Lares_Kopalnia_Condition;
	information	= ORG_801_Lares_Kopalnia_Info;
	permanent	= 0;
	important   = 1;
};                       

FUNC int ORG_801_Lares_Kopalnia_Condition()
{
...
FUNC VOID ORG_801_Lares_Kopalnia_Info()
{


     AI_Output (self, other,"ORG_801_Lares_Kopalnia_11_01"); //Musimy porozmawiaæ. Kroi siê przed nami zadanie!
	 AI_Output (other, self,"ORG_801_Lares_Kopalnia_15_02"); //Jakie?
	 AI_Output (self, other,"ORG_801_Lares_Kopalnia_11_03"); //Ostatnio w Starej Kopalni dochodzi do wielu wypadków. Te ich machiny potrzebne do wydobycia ci¹gle siê psuj¹.
	 AI_Output (self, other,"ORG_801_Lares_Kopalnia_11_04"); //W interesie Nowego Obozu tkwi by ten stan trwa³ jak najd³u¿ej. W koñcu os³abia to ca³y Stary Obóz a zw³aszcza Magnatów.
	
	
	    Info_ClearChoices (ORG_801_Lares_Kopalnia);
	    Info_AddChoice    (ORG_801_Lares_Kopalnia,"Jakie konkretne dzia³ania masz na myœli?",ORG_801_Lares_Dzia);
	   
	
};


FUNC VOID ORG_801_Lares_Dzia()
  {
	AI_Output (other,self ,"ORG_801_Lares_Dzia_15_00");  //Jakie konkretne dzia³ania masz na myœli?
	AI_Output (self ,other,"ORG_801_Lares_Dzia_11_01");  //A to ju¿ zale¿y od tego czego siê dowiesz. Udaj siê do obozu przed Star¹ Kopalni¹ i poszukaj informacji, które bêd¹ przydatne.
	AI_Output (other,self ,"ORG_801_Lares_Dzia_15_02");  //Tak zrobiê.
	MIS_Zawirowania = LOG_RUNNING;
	
	   Log_CreateTopic          (Zawirowania, LOG_MISSION);
       Log_SetTopicStatus       (Zawirowania, LOG_RUNNING);
	    AI_StopProcessInfos	(self);
	  B_LogEntry                     (Zawirowania,"Ostatnimi czasy dochodzi do zbêdnego zamieszania w Starej Kopaln co utrudnia wszelkie pracê nad wydobyciem. Lares jest zainteresowany by to trwa³o jak najd³u¿ej. Chcê bym siê temu przyjrza³. Mam udaæ siê do obozowiska przed Star¹ Kopalni¹ i dowiedzieæ siê co i jak.");
};
...
INSTANCE DIA_Lares_Mine (C_INFO)
{
   npc          = ORG_801_Lares;
   nr           = 1;
   condition    = DIA_Lares_Mine_Condition;
   information  = DIA_Lares_Mine_Info;
   permanent	= FALSE;
   description	= "Wiem ju¿ co planuj¹ Magnaci.";
};

FUNC INT DIA_Lares_Mine_Condition()
{
    if (Npc_KnowsInfo (hero, DIA_Patrick_Mine))
    {
    return TRUE;
    };
};
...
FUNC VOID DIA_Lares_Mine_Info()
{
    AI_Output (other, self ,"DIA_Lares_Mine_15_01"); //Wiem ju¿ co planuj¹ Magnaci.
    AI_Output (self, other ,"DIA_Lares_Mine_03_02"); //A wiêc?
    AI_Output (other, self ,"DIA_Lares_Mine_15_03"); //Chc¹ przys³aæ do Starej Kopalni jakiegoœ in¿yniera. Dziêki niemu wszystko ma zostaæ naprawione.
    AI_Output (self, other ,"DIA_Lares_Mine_03_04"); //Zatem ju¿ wiem co zrobimy. Ten ca³y fachowiec nie mo¿e dotrzeæ do Starej Kopalni. 
    AI_Output (self, other ,"DIA_Lares_Mine_03_05"); //Musimy siê dowiedzieæ kiedy wyrusza konwój, który bêdzie go eskortowa³ ze Starego Obozu a¿ do kopalni.
    AI_Output (self, other ,"DIA_Lares_Mine_03_06"); //Porozmawiaj z Mordragiem. Ju¿ dawno mia³ za³atwiæ w Starym Obozie informatora, który bêdzie nam przekazywa³ takie wiadomoœci.
    AI_Output (other, self ,"DIA_Lares_Mine_15_07"); //Dobrze.
    B_LogEntry                     (Zawirowania,"Lares kaza³ mi siê dowiedzieæ kiedy wyruszy konwój eskortuj¹cy in¿yniera do Starej Kopalni. Mam pomówiæ z Mordragiem bowiem ten mia³ pozyskaæ informatora w Starym Obozie.");
 AI_StopProcessInfos	(self);
    B_GiveXP (50);
...
INSTANCE DIA_Lares_Konwoj (C_INFO)
{
   npc          = ORG_801_Lares;
   nr           = 1;
   condition    = DIA_Lares_Konwoj_Condition;
   information  = DIA_Lares_Konwoj_Info;
   permanent	= FALSE;
   description	= "Konwój z in¿ynierem wyrusza jutro o 13!";
};

FUNC INT DIA_Lares_Konwoj_Condition()
{
    if (Npc_KnowsInfo (hero, DIA_Uscan_Mine1))
    {
    return TRUE;
    };
};
...
FUNC VOID DIA_Lares_Konwoj_Info()
{
    AI_Output (other, self ,"DIA_Lares_Konwoj_15_01"); //Konwój z in¿ynierem wyrusza jutro!
    AI_Output (self, other ,"DIA_Lares_Konwoj_03_02"); //Trzeba dzia³aæ szybko. Pogadaj z Roscoe. Powie ci, którzy Szkodnicy napadn¹ na tê eskortê.
    MIS_KONWOJLARESA = LOG_RUNNING;
    Konwoj_Day = Wld_GetDay(); //= Wld_GetDay();   
...
INSTANCE DIA_Lares_Inzy (C_INFO)
{
   npc          = ORG_801_Lares;
   nr           = 1;
   condition    = DIA_Lares_Inzy_Condition;
   information  = DIA_Lares_Inzy_Info;
   permanent	= FALSE;
   description	= "In¿ynier nie ¿yje.";
};

FUNC INT DIA_Lares_Inzy_Condition()
{
    if (Npc_IsDead(VLK_40044_Inzynier))
	&& (Npc_KnowsInfo (hero, DIA_CIPHER_ATAK_LAS1))
    {
    return TRUE;
    };
};
...
FUNC VOID DIA_Lares_Inzy_Info()
{
    AI_Output (other, self ,"DIA_Lares_Inzy_15_01"); //In¿ynier nie ¿yje.
    AI_Output (self, other ,"DIA_Lares_Inzy_03_02"); //Œwietna robota ch³opcze. Wiedzia³em, ¿e zdolny z ciebie goœæ. Zas³u¿y³eœ na niez³¹ nagrodê. Tylko nie przepij ca³ej od razu hehe!.
    AI_Output (other, self ,"DIA_Lares_Inzy_15_03"); //Dziêki. Starczy i na flaszkê i na inne sprawy.
        CreateInvItems (self, ItMiNugget, 300);
        B_GiveInvItems (self, other, ItMiNugget, 300);
    B_LogEntry                     (Zawirowania,"Lares by³ uradowany z rezultatu mej misji i od razu wyp³aci³ mi 300 bry³ek rudy. Nic tak nie cieszy w Nowym Obozie jak pora¿ka Gomeza.");
    AI_StopProcessInfos	(self);
    Log_SetTopicStatus       (Zawirowania, LOG_SUCCESS);
   B_GiveXP (350);
};
...
INSTANCE DIA_Lares_InzyFail (C_INFO)
{
   npc          = ORG_801_Lares;
   nr           = 1;
   condition    = DIA_Lares_InzyFail_Condition;
   information  = DIA_Lares_InzyFail_Info;
   permanent	= FALSE;
   important    = TRUE;
};

FUNC INT DIA_Lares_InzyFail_Condition()
{
    if (MIS_Zawirowania == LOG_FAILED)
    {
    return TRUE;
    };
};
...
FUNC VOID DIA_Lares_InzyFail_Info()
{

    AI_Output (self, other ,"DIA_Lares_InzyFail_03_00"); //Zawiod³eœ mnie. In¿ynier dotar³ do celu.
    AI_Output (other, self ,"DIA_Lares_InzyFail_15_01"); //Wybacz. Pewne sprawy potoczy³y siê nie tak jak siê spodziewa³em.
	AI_Output (self, other ,"DIA_Lares_InzyFail_03_02"); //Ma³o mnie to obchodzi. In¿ynier poczyni³ ju¿ rozpoczêcie napraw.
	AI_Output (self, other ,"DIA_Lares_InzyFail_03_03"); //Chaos w starej kopalni dobiega koñca. 
	AI_Output (other, self ,"DIA_Lares_InzyFail_15_04"); //Przykro mi.
	AI_Output (self, other ,"DIA_Lares_InzyFail_03_05"); //Mi równie¿. Bo nie dostaniesz pêkniêtej bry³ki rudy!
      
    B_LogEntry                     (Zawirowania,"Cholera. In¿ynier mi siê wymkn¹³. Lares by³ tym faktem bardzo zawiedziony. Oczywiœcie nie dosta³em ani kruszynki rudy za ca³¹ tê fatygê. Niech to diabli!");
    AI_StopProcessInfos	(self);
    Log_SetTopicStatus       (Zawirowania, LOG_FAILED);
    B_GiveXP (50);
};
