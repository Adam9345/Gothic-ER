...
INSTANCE DIA_Gravo_Oprawca (C_INFO)
{
   npc          = Vlk_572_Gravo;
   nr           = 1;
   condition    = DIA_Gravo_Oprawca_Condition;
   information  = DIA_Gravo_Oprawca_Info;
   permanent	= FALSE;
   description	= "Muszê pomówiæ ze Skipem.";
};

FUNC INT DIA_Gravo_Oprawca_Condition()
{
if (Npc_KnowsInfo(hero,DIA_Alberto_Oprawca_Ruda)) &&  (!Npc_KnowsInfo(hero,DIA_THORUS_WEJSCIE_RING_FIRE)) && (Npc_GetTrueGuild (other)!=GIL_STT)
{
    return TRUE;
};
};
FUNC VOID DIA_Gravo_Oprawca_Info()
{
    AI_Output (other, self ,"DIA_Gravo_Oprawca_15_01"); //Muszê pomówiæ ze Skipem.
    AI_Output (self, other ,"DIA_Gravo_Oprawca_03_02"); //A ja mogê ci to za³atwiæ, nie za darmo oczywiœcie.
    AI_Output (other, self ,"DIA_Gravo_Oprawca_15_03"); //A ile za to chcesz?
    AI_Output (self, other ,"DIA_Gravo_Oprawca_03_04"); //100 bry³ek.
    B_LogEntry               (Oprawca,"Gravo chcê 100 bry³ek za umówienie spotkania ze Skipem.");
};


Func void Skip_Oprawca()
{
if (Skip_OPR_Day<=(Wld_GetDay()-1)) 
{

Npc_ExchangeRoutine(GRD_211_Skip,"Oprawca");
};
};
...
INSTANCE DIA_Gravo_Oprawca_Nugget (C_INFO)
{
   npc          = Vlk_572_Gravo;
   nr           = 2;
   condition    = DIA_Gravo_Oprawca_Nugget_Condition;
   information  = DIA_Gravo_Oprawca_Nugget_Info;
   permanent	= FALSE;
   description	= "Masz. Oto 100 bry³ek rudy.";
};

FUNC INT DIA_Gravo_Oprawca_Nugget_Condition()
{
if (Npc_KnowsInfo(hero,DIA_Gravo_Oprawca)) && (Npc_HasItems (other, ItMiNugget) >=100)
{
    return TRUE;
};
};
FUNC VOID DIA_Gravo_Oprawca_Nugget_Info()
{
    AI_Output (other, self ,"DIA_Gravo_Oprawca_Nugget_15_01"); //Masz. Oto 100 bry³ek rudy.
	
	Skip_OPR_Day<=(Wld_GetDay()-1);
	Skip_Oprawca();
    AI_Output (self, other ,"DIA_Gravo_Oprawca_Nugget_03_02"); //Dobrze. PrzyjdŸ tu jutro, Skip powinien ju¿ tu na ciebie czekaæ.
	
	B_LogEntry               (Oprawca,"Da³em Gravo rudê, któr¹ chcia³, teraz mam poczekaæ do jutra a Skip bêdzie na mnie czeka³.");
...
INSTANCE DIA_Gravo_Zjawa (C_INFO)
{
   npc          = Vlk_572_Gravo;
   nr           = 1;
...
{
	if (Npc_KnowsInfo(hero,DIA_Glest_Zjawa_2))
{
    return TRUE;
};
};
FUNC VOID DIA_Gravo_Zjawa_Info()
{
    ...
    AI_Output (other, self ,"DIA_Gravo_Zjawa_15_05"); //A wiêc mamy œwiadka. Muszê z nim pogadaæ.
    AI_Output (self, other ,"DIA_Gravo_Zjawa_03_06"); //W tej sytuacji takie relacje s¹ bardzo wa¿ne. Choæ nadal bêdzie ci ciê¿ko ustaliæ sk¹d wziê³a siê ta zjawa.
...
INSTANCE DIA_Gravo_LSZ_Adolf (C_INFO)
{
   npc          = Vlk_572_Gravo;
   nr           = 1;
   condition    = DIA_Gravo_LSZ_Adolf_Condition;
   information  = DIA_Gravo_LSZ_Adolf_Info;
   permanent	= FALSE;
   description	= "Poszukuje niejakiego Adolfa.";
};

FUNC INT DIA_Gravo_LSZ_Adolf_Condition()
{
if (Npc_KnowsInfo(hero,DIA_Raven_LSZ_Adolf))
{
    return TRUE;
};
};
FUNC VOID DIA_Gravo_LSZ_Adolf_Info()
{
    AI_Output (other, self ,"DIA_Gravo_LSZ_Adolf_15_01"); //Poszukuje niejakiego Adolfa.
    AI_Output (self, other ,"DIA_Gravo_LSZ_Adolf_03_02"); // A po co ci kontakt z kimœ takim?
    AI_Output (other, self ,"DIA_Gravo_LSZ_Adolf_15_03"); //S³ysza³em, ¿e to wa¿ny i wp³ywowy cz³owiek w Starym Obozie.
    AI_Output (self, other ,"DIA_Gravo_LSZ_Adolf_03_04"); //Nie wiem kto naopowiada³ ci takich g³upot ch³opczê. Tracisz tylko czas.
    AI_Output (other, self ,"DIA_Gravo_LSZ_Adolf_15_05"); //To kim jest ten Adolf?
    AI_Output (self, other ,"DIA_Gravo_LSZ_Adolf_03_06"); // Teraz nikt go ju¿ nie nazywa.
    AI_Output (other, self ,"DIA_Gravo_LSZ_Adolf_15_07"); //To jak go zw¹?
    AI_Output (self, other ,"DIA_Gravo_LSZ_Adolf_03_08"); //Teraz to Wrzód. Skoñczony wariat i cholerny pata³ach... Tak miêdzy nami.
    AI_Output (other, self ,"DIA_Gravo_LSZ_Adolf_15_09"); //Em... Tak... To ja ju¿ sobie pójdê.
	B_LogEntry               (LatwySzmal,"Gravo ujawni³ smutn¹ prawdê. Adolf to prawdziwe imiê cz³owieka o wiele mówi¹cym pseudonimie Wrzód. Dam mu t¹ wiadomoœæ od Ziggy’ego. Niech stracê.");
    AI_StopProcessInfos	(self);
};
