...
FUNC void  GRD_200_Thorus_TEACH_2H_START_Info()
{
	...
	Info_AddChoice(GRD_200_Thorus_TEACH_2H_START,"Wróæ",DIA_Thorus_START_TRAIN_BACK);
	
	Info_AddChoice(GRD_200_Thorus_TEACH_2H_START,B_BuildLearnString("Broñ dwurêczna +1",B_GetLearnCostTalent(other,NPC_TALENT_2H,1),0),DIA_Thorus_START_TRAIN_2H_1);
	Info_AddChoice(GRD_200_Thorus_TEACH_2H_START,B_BuildLearnString("Broñ dwurêczna +5",B_GetLearnCostTalent(other,NPC_TALENT_2H,5),0),DIA_Thorus_START_TRAIN_2H_5);
...
func void DIA_Thorus_START_TRAIN_BACK()
{
	Info_ClearChoices(GRD_200_Thorus_TEACH_2H_START);
};
...
INSTANCE DIA_Thorus_Potwor (C_INFO)
{
   npc          = GRD_200_THORUS;
   nr           = 1;
   condition    = DIA_Thorus_Potwor_Condition;
   information  = DIA_Thorus_Potwor_Info;
   permanent	= FALSE;
   important    = TRUE;
};

FUNC INT DIA_Thorus_Potwor_Condition()
{
    if (Npc_GetTrueGuild(hero) == GIL_GRD)
	&& (MIS_ReplacePointDefense == LOG_SUCCESS)
	&& (ThorusNewQuest_Day != Wld_GetDay())
...
FUNC VOID DIA_Thorus_Potwor_Info()
{
    AI_Output (self, other ,"DIA_Thorus_Potwor_03_01"); //Mam dla ciebie nastêpne zadanie. 
	AI_Output (other, self ,"DIA_Thorus_Potwor_15_02"); //Jakie?
    AI_Output (self, other ,"DIA_Thorus_Potwor_03_03"); //Nie doœæ ¿e bandyci napadli na plac wymian, to kopacze nie chc¹ s³u¿yæ w tamto miejsce jako tragarze.
	AI_Output (other, self ,"DIA_Thorus_Potwor_15_04"); //Dlaczego?
	AI_Output (self, other ,"DIA_Thorus_Potwor_03_05"); //Tym razem nie t³umaczy³bym tego ich leniwym usposobieniem.
	AI_Output (self, other ,"DIA_Thorus_Potwor_03_06"); //Chodzi o coœ innego. Ponoæ grasuje tam jakaœ bestia.
	AI_Output (other, self ,"DIA_Thorus_Potwor_15_07"); //Bestia?
	AI_Output (self, other ,"DIA_Thorus_Potwor_03_08"); //Ponoæ tak. Niejaki Patrick z obozu przed star¹ kopalni¹ widzia³ ponoæ tego potwora.
	AI_Output (self, other ,"DIA_Thorus_Potwor_03_09"); //Masz z nim o tym pomówiæ.
	AI_Output (other, self ,"DIA_Thorus_Potwor_15_10"); //Zajmê siê tym.
	AI_Output (self, other ,"DIA_Thorus_Potwor_03_11"); //Prawid³owo. Powodzenia.
	
	
    Log_CreateTopic          (PotworBramus, LOG_MISSION);
    Log_SetTopicStatus       (PotworBramus, LOG_RUNNING);
	B_LogEntry               (PotworBramus ,"Kopacze boj¹ siê udawaæ w stronê placu wymian by dostarczaæ towary z obozu. Ponoæ jest tam jakaœ bestia. Muszê pomówiæ z Patrickiem z obozowiska przed kopalni¹. On ponoæ widzia³ tego stwora.");
};
...
INSTANCE DIA_Thorus_Potwor_Joel (C_INFO)
{
   npc          = GRD_200_THORUS;
   nr           = 1;
   condition    = DIA_Thorus_Potwor_Joel_Condition;
   information  = DIA_Thorus_Potwor_Joel_Info;
   permanent	= FALSE;
   description  = "Chcia³em zdaæ raport odnoœnie œledztwa...";
};

FUNC INT DIA_Thorus_Potwor_Joel_Condition()
{
    if  (Npc_KnowsInfo (hero,  DIA_Joel_Potwor))
    {
    return TRUE;
    };
};
...
FUNC VOID DIA_Thorus_Potwor_Joel_Info()
{
    AI_Output (other, self ,"DIA_Thorus_Potwor_Joel_15_01");//Chcia³em zdaæ raport odnoœnie œledztwa dotycz¹cego wielkiej bestii.	
    AI_Output (self, other ,"DIA_Thorus_Potwor_Joel_03_02"); //Mów.
	AI_Output (other, self ,"DIA_Thorus_Potwor_Joel_15_03"); //Potwór zadaje wielkie obra¿enia czego doœwiadczy³ jakiœ nieszczêœnik. Jego zw³oki widzia³ stra¿nik Joel.
	AI_Output (self, other ,"DIA_Thorus_Potwor_Joel_03_04"); //A czy naprawdê widziano sam¹ bestiê?
	AI_Output (other, self ,"DIA_Thorus_Potwor_Joel_15_05"); //Chodzi o coœ innego. Ponoæ grasuje tam jakaœ bestia.
	AI_Output (other, self ,"DIA_Thorus_Potwor_Joel_15_06"); //S¹dzê, ¿e Patrick nie k³amie. Widzia³ stwora noc¹ kiedy ¿erowa³.
	AI_Output (self, other ,"DIA_Thorus_Potwor_Joel_03_07"); //W takim razie pomów z Fletcherem i Siekaczem. Noc¹ udacie siê na plac wymian.
	AI_Output (self, other ,"DIA_Thorus_Potwor_Joel_03_08"); //Macie zabiæ tego potwora.
	AI_Output (other, self ,"DIA_Thorus_Potwor_Joel_15_09"); //W porz¹dku.
	AI_Output (self, other ,"DIA_Thorus_Potwor_Joel_03_10"); //Zatem wiesz co masz robiæ.
	
	
	
	B_LogEntry               (PotworBramus ,"Rozmawia³em z Thorusem i powiedzia³em jak ma siê sytuacja. Poleci³ mi pogadaæ z Fletcherem i Siekaczem a nastêpnie noc¹ udaæ siê z nimi na plac wymian by zabiæ potwora.");
	Wld_InsertNpc (HugeSnapper, "OW_PATH_1_5_3");
};
...
INSTANCE DIA_Thorus_Potwor_Joel1 (C_INFO)
{
   npc          = GRD_200_THORUS;
   nr           = 1;
   condition    = DIA_Thorus_Potwor_Joel1_Condition;
   information  = DIA_Thorus_Potwor_Joel1_Info;
   permanent	= FALSE;
   description  = "Potwór sprzed placu wymian nie ¿yje.";
};

FUNC INT DIA_Thorus_Potwor_Joel1_Condition()
{
    if (Npc_IsDead(HugeSnapper)) 
	&& (Npc_KnowsInfo (hero,  DIA_Cutter_Bramus_Potwor))
    {
    return TRUE;
    };
};
...
FUNC VOID DIA_Thorus_Potwor_Joel1_Info()
{
    AI_Output (other, self ,"DIA_Thorus_Potwor_Joel1_15_01"); //Potwór sprzed placu wymian nie ¿yje.
    AI_Output (self, other ,"DIA_Thorus_Potwor_Joel1_03_02"); //Œwietnie. Teraz nasi ludzie bêd¹ bezpieczni i przestan¹ biadoliæ o tym zagro¿eniu.
	AI_Output (self, other ,"DIA_Thorus_Potwor_Joel1_03_03"); //Dobrze siê spisa³eœ, jak na ¿ó³todzioba.
	AI_Output (other, self ,"DIA_Thorus_Potwor_Joel1_15_04"); //Co z nagrod¹?
	AI_Output (self, other ,"DIA_Thorus_Potwor_Joel1_03_05"); //A tak... Oto twoja ruda... I taki ma³y drobiazg.
	AI_Output (other, self ,"DIA_Thorus_Potwor_Joel1_15_06"); //Dziêki. W razie gdybym by³ potrzebny to jestem w pobli¿u.
	AI_Output (self, other ,"DIA_Thorus_Potwor_Joel1_03_07"); //Tak jasne, nie przepij wszystkiego.
	AI_Output (other, self ,"DIA_Thorus_Potwor_Joel1_15_08"); //Na razie.
   B_GiveInvItems (self,other, itminugget, 150);
   B_GiveInvItems (self,other, ItBE_WAR, 1);
   B_LogEntry               (PotworBramus ,"Powiedzia³em Thorusowi o nocnej walce i ubiciu ogromnego potwora. By³ zadowolony z rezultatu misji i wyp³aci³ mi nagrodê.");
   Log_SetTopicStatus       (PotworBramus, LOG_SUCCESS);
...
//-----------------> WHAT_YOU_DOING
//
//Opcja z pancerzem od Pascala
//========================================

INSTANCE DIA_THORUS_WHAT_YOU_DOING (C_INFO)
{
   npc          = GRD_200_THORUS;
   nr           = 1;
   condition    = DIA_THORUS_WHAT_YOU_DOING_Condition;
   information  = DIA_THORUS_WHAT_YOU_DOING_Info;
   permanent	= FALSE;
   Important    = TRUE;
};

FUNC INT DIA_THORUS_WHAT_YOU_DOING_Condition()
{
	...
	//&& (PP_Mission == LOG_RUNNING)
{
    return TRUE;
};
};
FUNC VOID DIA_THORUS_WHAT_YOU_DOING_Info()
{
    AI_Output (self, other ,"DIA_THORUS_WHAT_YOU_DOING_03_01"); //Chyba sobie ¿artujesz.
    AI_Output (other, self ,"DIA_THORUS_WHAT_YOU_DOING_15_02"); //Jakiœ problem?
    AI_Output (self, other ,"DIA_THORUS_WHAT_YOU_DOING_03_03"); //Chcesz zrobiæ ze mnie g³upca? 
    AI_Output (self, other ,"DIA_THORUS_WHAT_YOU_DOING_03_04"); //Co to za pancerz? Nie przypominam sobie byœ do³¹cza³ w szeregi cieni. Jeœli dowiem siê ¿e zabi³eœ któregoœ z nich to gwarantuje ci ¿e masz przechlapane.
    AI_Output (self, other ,"DIA_THORUS_WHAT_YOU_DOING_03_05"); //Nie doœæ ¿e uwa¿asz ¿e jestem tak g³upi, to jeszcze pchasz mi siê do zamku! W ty³ zwrot i wynocha, chyba ¿e chcesz ¿ebym siê zdenerwowa³.
};
