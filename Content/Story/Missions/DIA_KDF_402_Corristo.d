...
FUNC int  Info_Corristo_Intruder_Condition()
{
	...
	&& (!Npc_KnowsInfo(hero,DIA_PC_MAGE_PRZESYLKA))
	&& (kapitel < 4) 
...
FUNC VOID DIA_Corristo_QUestSucces_Info()
{
    ...
    AI_Output (self, other ,"DIA_Corristo_QUestSucces_03_06"); //Przeklêty bandzior. Bêdê siê modli³ do Innosa, by go ukara³. 
...
func void  KDF_402_Corristo_ROBE_Info()
{
	...
	B_LogEntry		(GE_TeacherOC,"Corristo mo¿e mnie wprowadziæ w krêgi magii oraz zwiêkszyæ moje zdolnoœci magiczne. Znajdê go w siedzibie Magów Ognia.");	
};
//--------------------------------------------------------------------------
// 							MANA KAUFEN
//--------------------------------------------------------------------------
...
INSTANCE DIA_Corristo_Przesylka (C_INFO)
{
   npc          = KDF_402_Corristo;
   nr           = 1;
   condition    = DIA_Corristo_Przesylka_Condition;
   information  = DIA_Corristo_Przesylka_Info;
   permanent	= FALSE;
   description	= "Witaj arcymistrzu. Ponoæ czegoœ ode mnie chcesz.";
};

FUNC INT DIA_Corristo_Przesylka_Condition()
{
    if (Npc_KnowsInfo (hero, DIA_PC_MAGE_PRZESYLKA))
...
FUNC VOID DIA_Corristo_Przesylka_Info()
{
    AI_Output (other, self ,"DIA_Corristo_Przesylka_15_01"); //Witaj arcymistrzu. Ponoæ czegoœ ode mnie chcesz.
    AI_Output (self, other ,"DIA_Corristo_Przesylka_03_02"); //Oczekuje pewnej cennej przesy³ki z zewnêtrznego œwiata.
    AI_Output (self, other ,"DIA_Corristo_Przesylka_03_03"); //Powinienem j¹ otrzymaæ ju¿ jakiœ czas temu. Jednak wci¹¿ jej nie mam.
    AI_Output (other, self ,"DIA_Corristo_Przesylka_15_04"); //Co mia³bym w zwi¹zku z tym zrobiæ?
    AI_Output (self, other ,"DIA_Corristo_Przesylka_03_05"); //Udaæ siê na plac wymian i dowiedzieæ siê dlaczego przesy³ka wci¹¿ do mnie nie dotar³a.
    AI_Output (other, self ,"DIA_Corristo_Przesylka_15_06"); //Rozumiem. Zajmê siê tym.
    AI_Output (self, other ,"DIA_Corristo_Przesylka_03_07"); //Za³atw tê sprawê a nagroda ciê nie ominie.
    AI_Output (other, self ,"DIA_Corristo_Przesylka_15_08"); //Oczywiœcie.
	
	
      B_LogEntry                     (PrzesylkaCorristo ,"Corristo oczekuje na pewn¹ przesy³kê z zewnêtrznego œwiata. Powinien ju¿ j¹ dostaæ a z nie znanych przyczyn tak siê nie dzieje. Mam udaæ siê na plac wymian i dowiedzieæ siê co z paczk¹ dla Corristo.");
    AI_StopProcessInfos	(self);
};
...
INSTANCE DIA_Corristo_Przesylka1 (C_INFO)
{
   npc          = KDF_402_Corristo;
   nr           = 1;
   condition    = DIA_Corristo_Przesylka1_Condition;
   information  = DIA_Corristo_Przesylka1_Info;
   permanent	= FALSE;
   description	= "Mam twoj¹ przesy³kê arcymistrzu.";
};

FUNC INT DIA_Corristo_Przesylka1_Condition()
{
    if (Npc_KnowsInfo (hero, DIA_PC_MAGE_PRZESYLKA))
...
FUNC VOID DIA_Corristo_Przesylka1_Info()
{
    AI_Output (other, self ,"DIA_Corristo_Przesylka1_15_01"); //Mam twoj¹ przesy³kê arcymistrzu.
    AI_Output (self, other ,"DIA_Corristo_Przesylka1_03_02"); //No nareszcie.
    AI_Output (other, self ,"DIA_Corristo_Przesylka1_15_03"); //By³y pewne komplikacje. Po drodze wpad³a w rêce rozbójników, którzy okradli plac wymian.
    AI_Output (self, other ,"DIA_Corristo_Przesylka1_03_04"); //Co?! Ale jest w nienaruszonym stanie?  
    AI_Output (other, self ,"DIA_Corristo_Przesylka1_15_05"); //Na to wygl¹da.
    AI_Output (self, other ,"DIA_Corristo_Przesylka1_03_06"); //Szczêœcie. W koñcu j¹ mam.
    AI_Output (other, self ,"DIA_Corristo_Przesylka1_15_07"); //Co to za przesy³ka?
    AI_Output (self, other ,"DIA_Corristo_Przesylka1_03_08"); //To ju¿ moja sprawa. 
	AI_Output (other, self ,"DIA_Corristo_Przesylka1_15_09"); //Rozumiem.
	AI_Output (self, other ,"DIA_Corristo_Przesylka1_03_10"); //Jestem zajêty, chyba ju¿ czas na ciebie...
	
	
	B_GiveInvItems (other,self, ItMi_Corristo_pack, 1);
    B_LogEntry                     (PrzesylkaCorristo ,"Odda³em przesy³kê Corristo i poda³em przyczynê opóŸnienia w jej dotarciu do jego r¹k. By³ zadowolony a jednoczeœnie doœæ tajemniczy. W ka¿dym razie wyp³aci³ mi stosown¹ nagrodê i moja misja jest ukoñczona.");
	 ...
	 Info_ClearChoices (DIA_Corristo_Przesylka1);
	 Info_AddChoice    (DIA_Corristo_Przesylka1,"Chcia³bym nieœmia³o wspomnieæ o nagrodzie...",DIA_Corristo_Cienw);
};

FUNC VOID DIA_Corristo_Cienw()
  {
	AI_Output (other,self ,"DIA_Corristo_Cienw_15_00");  //Chcia³bym nieœmia³o wspomnieæ o nagrodzie.
	AI_Output (self ,other,"DIA_Corristo_Cienw_11_01");  //A by³bym zapomnia³... Oto twoja nagroda.
	AI_Output (other,self ,"DIA_Corristo_Cienw_15_02");  //Dziêkuje.
    AI_Output (self ,other,"DIA_Corristo_Cienw_11_03");  //Postaraj siê wydaæ tê rudê i u¿yæ tych zwojów, w miarê s³usznym celu.
	AI_Output (other,self ,"DIA_Corristo_Cienw_15_04");  //Tak bêdzie.
    AI_StopProcessInfos	     (self);
    Log_SetTopicStatus       (PrzesylkaCorristo , LOG_SUCCESS);
...
INSTANCE DIA_Corristo_Jadro (C_INFO)
{
   npc          = KDF_402_Corristo;
   nr           = 1;
   condition    = DIA_Corristo_Jadro_Condition;
   information  = DIA_Corristo_Jadro_Info;
   permanent	= FALSE;
   important    = TRUE;
};

FUNC INT DIA_Corristo_Jadro_Condition()
{
     if (Npc_GetTrueGuild(hero) == GIL_KDF)
...
FUNC VOID DIA_Corristo_Jadro_Info()
{
    AI_Output (self, other ,"DIA_Corristo_Jadro_03_01"); //Mam dla ciebie wa¿n¹ misje. Chodzi o pewien artefakt.
    AI_Output (self, other ,"DIA_Corristo_Jadro_03_02"); //Zw¹ go j¹drem ognia. To magiczny kamieñ. Ma wa¿n¹ i cenn¹ dla nas Magów moc, któr¹ powinniœmy pozyskaæ.
	AI_Output (other, self ,"DIA_Corristo_Jadro_15_03"); //Gdzie znajdê ten artefakt?
    ...
    AI_Output (self, other ,"DIA_Corristo_Jadro_03_07"); //Im szybciej, tym lepiej. IdŸ i zdob¹dŸ Kryszta³ ognia dla naszego krêgu.
      ...
      B_LogEntry                 (JadroOgnia ,"Corristo chce bym zdoby³ artefakt zwany j¹drem ognia. Ponoæ Milten ma informacje, które pomog¹ nam go zlokalizowaæ. Muszê z nim pogadaæ.");
      AI_StopProcessInfos	     (self);
};
var int JO_END;
//========================================
...
INSTANCE DIA_Corristo_JajoOgnia (C_INFO)
{
   npc          = KDF_402_Corristo;
   nr           = 1;
...
{
    if (Npc_HasItems (other, ItMi_Focus_Corristo) >=1)
...
FUNC VOID DIA_Corristo_JajoOgnia_Info()
{
    ...
    AI_Output (other, self ,"DIA_Corristo_JajoOgnia_15_05"); //Proszê, oto on.
    AI_Output (self, other ,"DIA_Corristo_JajoOgnia_03_06"); //Teraz artefakt trafi³ we w³aœciwe rêce. 
    AI_Output (self, other ,"DIA_Corristo_JajoOgnia_03_07"); //Zas³u¿y³eœ na nagrodê. WeŸ te kilka mikstur. 
    AI_Output (other, self ,"DIA_Corristo_JajoOgnia_15_08"); //Dziêkuje, Arcymistrzu.
	AI_Output (self, other ,"DIA_Corristo_JajoOgnia_03_09"); //Zajmij siê teraz swoimi sprawami i niech Innos ciê prowadzi.
	AI_Output (self, other ,"DIA_Corristo_JajoOgnia_03_10"); //Jestem zajêty, chyba ju¿ czas na ciebie...
...
INSTANCE DIA_Corristo_HELLO1 (C_INFO)
{
   npc          = KDF_402_Corristo;
   nr           = 1;
   condition    = DIA_Corristo_HELLO1_Condition;
   information  = DIA_Corristo_HELLO1_Info;
   permanent	= FALSE;
   description	= "Mam wa¿n¹ sprawê.";
};

FUNC INT DIA_Corristo_HELLO1_Condition()
{
if (B_Corristo_talk == TRUE)
&& (Npc_KnowsInfo(hero,DIA_John_HELLO1))
{
    return TRUE;
};
};
FUNC VOID DIA_Corristo_HELLO1_Info()
{
    AI_Output (other, self ,"DIA_Corristo_HELLO1_15_01"); //Mam wa¿n¹ sprawê.
	
    AI_Output (self, other ,"DIA_Corristo_HELLO1_03_02"); //Jestem zajêty. Mam nadzieje, ¿e masz wyraŸny powód by mi przeszkadzaæ.
    AI_Output (other, self ,"DIA_Corristo_HELLO1_15_03"); //Chcia³bym kupiæ zaklêcie zwane ,,Kling¹ Ognia”.
    AI_Output (self, other ,"DIA_Corristo_HELLO1_03_04"); //Co?! Byle kmiotek nie mo¿e mieæ w rêkach tak potê¿nego zaklêcia. To bezcelowe i szkodliwe. Nawet gdybyœ da³ mi za to tonê rudy.
    AI_Output (self, other ,"DIA_Corristo_HELLO1_03_05"); //ZejdŸ mi z oczu. Nie ma mowy.
	
    AI_Output (other, self ,"DIA_Corristo_HELLO1_15_06"); //Tylko, ¿e...
    AI_Output (self, other ,"DIA_Corristo_HELLO1_03_07"); //Precz!
	Npc_ExchangeRoutine(GRD_208_Cutter,"BLA");
	AI_Teleport(GRD_208_Cutter,"WP_SIEKACZ_BLA");
	B_LogEntry               (Blachostka,"Corristo oczywiœcie nie zgodzi³ siê na sprzeda¿ ,,Klingi Ognia”. Mog³em siê tego spodziewaæ. Co teraz?");
    AI_StopProcessInfos	(self);
};

instance dia_corristo_pickpocket(c_info) {
    npc = kdf_402_corristo;
    nr = 900;
    condition = dia_corristo_pickpocket_condition;
    information = dia_corristo_pickpocket_info;
    permanent = 1;
    description = pickpocket_final;
};

func int dia_corristo_pickpocket_condition() {
	e_beklauen(baseThfChanceKDF - 5, 50);
};

func void dia_corristo_pickpocket_info() {
	b_steal_message();
	info_clearchoices(dia_corristo_pickpocket);
	info_addchoice(dia_corristo_pickpocket, dialog_back, dia_corristo_pickpocket_back);
	info_addchoice(dia_corristo_pickpocket, dialog_pickpocket, dia_corristo_pickpocket_doit);
};

func void dia_corristo_pickpocket_doit() {
    d_beklauen();
    info_clearchoices(dia_corristo_pickpocket);
};

func void dia_corristo_pickpocket_back() {
    info_clearchoices(dia_corristo_pickpocket);
};
