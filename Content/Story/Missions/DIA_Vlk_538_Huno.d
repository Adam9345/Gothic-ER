...
FUNC VOID DIA_Huno_LEARNSMITH_Info()
{
	...
	AI_Output (self,other,"DIA_Huno_LEARNSMITH_09_01"); //Tylko nie wyobra¿aj sobie, ¿e tak ³atwo jest wykuæ solidny miecz! Jeœli chcesz mogê ciê nauczyæ konkretnie czegoœ o kowalstwie.
	...
	AI_Output (self,other,"DIA_Huno_LEARNSMITH_09_06"); //Na koniec trzeba je jeszcze naostrzyæ na kamieniu szlifierskim. To wszystko jeœli chodzi o wykucie prostego miecza.
...
INSTANCE DIA_Huno_Oprawca (C_INFO)
{
   ...
   permanent	= FALSE;
   description	= "Witaj. Obi³o ci siê o uszy imiê Winston?";
};
...
FUNC VOID DIA_Huno_LearnSmith_01_Info()
{
    ...
    if  (hero.lp >= 5) 
	{
    AI_Output (other, self ,"DIA_Huno_LearnSmith_01_15_01"); //Naucz mnie jak wykuæ miecz. 
    AI_Output (self, other ,"DIA_Huno_LearnSmith_01_03_02"); //To doœæ proste. ZnajdŸ sobie plan oraz kawa³ek surowej stali i rozgrzej go w ogniu, a¿ zacznie siê ¿arzyæ.
    AI_Output (self, other ,"DIA_Huno_LearnSmith_01_03_03"); //Potem wykuj na kowadle ostrze.
    AI_Output (self, other ,"DIA_Huno_LearnSmith_01_03_04"); //Najwa¿niejsze, ¿ebyœ pilnowa³, czy ostrze nie wystyg³o. Masz tylko kilka minut, ¿eby zrobiæ broñ...
    AI_Output (self, other ,"DIA_Huno_LearnSmith_01_03_05"); //Reszty dowiesz siê sam. To kwestia praktyki. 
	...
	{
	AI_Output (self,other ,"DIA_Huno_LearnSmith_01_03_06"); //Brak ci doœwiadczenia.
	PrintS_Ext  ("Brak ci doœwiadczenia!", COL_RED);
	};
	
};

instance dia_huno_pickpocket(c_info) {
    npc = vlk_538_huno;
    nr = 900;
    condition = dia_huno_pickpocket_condition;
    information = dia_huno_pickpocket_info;
    permanent = 1;
    description = pickpocket_final;
};

func int dia_huno_pickpocket_condition() {
	e_beklauen(baseThfChanceVLK, 20);
};

func void dia_huno_pickpocket_info() {
	b_steal_message();
	info_clearchoices(dia_huno_pickpocket);
	info_addchoice(dia_huno_pickpocket, dialog_back, dia_huno_pickpocket_back);
	info_addchoice(dia_huno_pickpocket, dialog_pickpocket, dia_huno_pickpocket_doit);
};

func void dia_huno_pickpocket_doit() {
    d_beklauen();
    info_clearchoices(dia_huno_pickpocket);
};

func void dia_huno_pickpocket_back() {
    info_clearchoices(dia_huno_pickpocket);
};
