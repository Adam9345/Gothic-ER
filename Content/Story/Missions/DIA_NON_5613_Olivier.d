...
FUNC VOID DIA_Olivier_HELLO6_Info()
{
	...
	NON_5613_Olivier.attribute[ATR_HITPOINTS] = 560;
...
INSTANCE DIA_Olivier_PNA_Udzial (C_INFO)
{
   npc          = NON_5613_Olivier;
   nr           = 2;
   condition    = DIA_Olivier_PNA_Udzial_Condition;
   information  = DIA_Olivier_PNA_Udzial_Info;
   permanent	= FALSE;
   description	=  "Masz wzi¹æ udzia³ w polowaniu na aligatory.";
};

FUNC INT DIA_Olivier_PNA_Udzial_Condition()
{
...
FUNC VOID DIA_Olivier_PNA_Udzial_Info()
{
    AI_Output (other, self ,"DIA_Olivier_PNA_Udzial_15_01"); //Masz wzi¹æ udzia³ w polowaniu na aligatory.
    AI_Output (self, other ,"DIA_Olivier_PNA_Udzial_03_02"); //Brzmi dobrze, zapowiada siê niez³y urobek.
    AI_Output (other,self  ,"DIA_Olivier_PNA_Udzial_15_03"); //Dok³adnie. Wchodzisz w to?
    AI_Output (self, other ,"DIA_Olivier_PNA_Udzial_03_04"); //Pewnie. Zaczekam przed obozem.
	Npc_ExchangeRoutine(NON_5613_Olivier,"WAITPNA");

	 B_LogEntry               (PolowanieNaAligatory,"Olivier z entuzjazmem odniós³ siê do propozycji wziêcia udzia³u w polowaniu na aligatory. Bez trudu przysta³ na moj¹ propozycjê.");
...
INSTANCE DIA_Olivier_PNA_GO_TO_HUNT (C_INFO)
{
   npc          = NON_5613_Olivier;
   nr           = 1;
   condition    = DIA_Olivier_PNA_GO_TO_HUNT_Condition;
   information  = DIA_Olivier_PNA_GO_TO_HUNT_Info;
   permanent	= FALSE;
   description	= "ChodŸmy zapolowaæ.";
};

FUNC INT DIA_Olivier_PNA_GO_TO_HUNT_Condition()
{
if (Npc_KnowsInfo(hero,DIA_Olivier_PNA_Udzial))
&& (Npc_GetDistToWP(NON_5613_Olivier,"WP_PNA_OLIVIER_WAIT")<300)
{
    return TRUE;
};
};
FUNC VOID DIA_Olivier_PNA_GO_TO_HUNT_Info()
{
    AI_Output (other, self ,"DIA_Olivier_PNA_GO_TO_HUNT_15_01"); //ChodŸmy zapolowaæ.
    AI_Output (self, other ,"DIA_Olivier_PNA_GO_TO_HUNT_03_02"); //W drogê.
	NON_5613_Olivier.aivar[AIV_PARTYMEMBER] = TRUE;
	Npc_ExchangeRoutine(NON_5613_Olivier,"FollowPNA");
};

instance dia_olivier_pickpocket(c_info) {
    npc = non_5613_olivier;
    nr = 900;
    condition = dia_olivier_pickpocket_condition;
    information = dia_olivier_pickpocket_info;
    permanent = 1;
    description = pickpocket_final;
};

func int dia_olivier_pickpocket_condition() {
	e_beklauen(baseThfChanceORG, 35);
};

func void dia_olivier_pickpocket_info() {
	b_steal_message();
	info_clearchoices(dia_olivier_pickpocket);
	info_addchoice(dia_olivier_pickpocket, dialog_back, dia_olivier_pickpocket_back);
	info_addchoice(dia_olivier_pickpocket, dialog_pickpocket, dia_olivier_pickpocket_doit);
};

func void dia_olivier_pickpocket_doit() {
    d_beklauen();
    info_clearchoices(dia_olivier_pickpocket);
};

func void dia_olivier_pickpocket_back() {
    info_clearchoices(dia_olivier_pickpocket);
};