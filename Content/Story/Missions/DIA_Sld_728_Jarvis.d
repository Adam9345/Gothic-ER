...
{
    AI_Output (self, other ,"DIA_Jarvis_PoBuncie_03_01"); //No nieŸle namiesza³eœ, synek.
    ...
    AI_Output (other, self ,"DIA_Jarvis_PoBuncie_15_04"); //Spokojnie, najgorsze za nami.
	//exp
    B_GiveXP (100);
	//jarvis
...
FUNC VOID DIA_Jarvis_Wiateryyyyy_Info()
{
    ...
    AI_Output (other, self ,"DIA_Jarvis_Wiateryyyyy_15_06"); //No nieŸle byœcie mnie urz¹dzili! 
...
INSTANCE DIA_Jarvis_Zerron (C_INFO)
{
   npc          = Sld_728_Jarvis;
   nr           = 1;
   condition    = DIA_Jarvis_Zerron_Condition;
   information  = DIA_Jarvis_Zerron_Info;
   permanent	= FALSE;
   Important    = TRUE;
};

FUNC INT DIA_Jarvis_Zerron_Condition()
{ 
if (Npc_KnowsInfo(hero,DIA_Jarvis_First))
{
    return TRUE;
...
FUNC VOID DIA_Jarvis_Zerron_Info()
{
    AI_Output (self, other ,"DIA_Jarvis_Zerron_03_01"); //A i jeszcze jedno. Mo¿esz zajrzeæ do mojego kumpla Zerrona. Siedzi na pobliskiej wie¿y.
    AI_Output (other, self ,"DIA_Jarvis_Zerron_15_02"); //Niby po co?
    AI_Output (self, other ,"DIA_Jarvis_Zerron_03_03"); //Ci¹gle œlêczy nad ksi¹¿kami. 
    AI_Output (other, self ,"DIA_Jarvis_Zerron_15_04"); //Wiêc mo¿e lepiej mu nie przeszkadzaæ.
    AI_Output (self, other ,"DIA_Jarvis_Zerron_03_05"); //Co za du¿o to niezdrowo. Jeœli nikt tego nie zrobi Zerron ca³kiem odleci œwiadomoœci¹ jak ptak w niebiosa. 
    AI_Output (self, other ,"DIA_Jarvis_Zerron_03_06"); //Wiêc mo¿e go odwiedŸ.
    AI_Output (other, self ,"DIA_Jarvis_Zerron_15_07"); //Tylko co mogê na tym zyskaæ?
    AI_Output (self, other ,"DIA_Jarvis_Zerron_03_08"); //Zerron skupuje ró¿ne ksiêgi. Mo¿e ciê za nie naprawdê sowicie wynagrodziæ. W koñcu to jego pasja.
    AI_Output (other, self ,"DIA_Jarvis_Zerron_15_09"); //To ju¿ bardziej przydatna informacja. Mo¿e z³o¿ê mu wizytê.
    AI_Output (self, other ,"DIA_Jarvis_Zerron_03_10"); //No I s³usznie.
	Log_CreateTopic (GE_TraderNC,LOG_NOTE);
	B_LogEntry		(GE_TraderNC,"Najemnik Zerron przesiaduj¹cy na wie¿y nad Jarvisem skupuje ró¿ne ksi¹¿ki, mo¿e mnie dobrze wynagrodziæ w zamian za jedn¹ z nich."); 
};
...
INSTANCE DIA_Jarvis_PZ (C_INFO)
{
   npc          = Sld_728_Jarvis;
   nr           = 1;
   condition    = DIA_Jarvis_PZ_Condition;
   information  = DIA_Jarvis_PZ_Info;
   permanent	= FALSE;
   description	= "Dosz³o do zamachu na Lee. ";
};

FUNC INT DIA_Jarvis_PZ_Condition()
{
if (Npc_KnowsInfo(hero,DIA_Podejrzany_HELLO1))
{
    return TRUE;
};
};
FUNC VOID DIA_Jarvis_PZ_Info()
{
