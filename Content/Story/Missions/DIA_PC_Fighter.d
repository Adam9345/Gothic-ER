...
func VOID Info_Gorn_ZASTEPSTWO_Info()
{
	...
	{
    Npc_ExchangeRoutine (SLD_40129_Pascal,"WTHZM");
	};
	AI_GotoNpc			(self, hero);
...
func void AttackNC_FW()
{
    Wld_InsertNpc				(GRD_40038_Gardist,"WP_ATTACKNC2"); //1g
    Wld_InsertNpc				(GRD_40040_Gardist,"WP_ATTACKNC1"); //1g
    Wld_InsertNpc				(GRD_40036_Gardist,"WP_ATTACKNC3"); //1g    
	Wld_InsertNpc				(GRD_40039_Gardist,"WP_ATTACKNC4"); //1g
	
	
	Wld_InsertNpc				(GRD_40037_Gardist,"WP_ATTACKNC5"); //2g
	Wld_InsertNpc				(GRD_40035_Gardist,"WP_ATTACKNC6"); //2g	
	Wld_InsertNpc				(GRD_40044_Gardist,"WP_ATTACKNC7"); //2g
	
	
	//WP_ATTACKNC8 ---- JIM
	Wld_InsertNpc				(GRD_40041_Gardist,"WP_ATTACKNC9" ); //3g
	Wld_InsertNpc				(GRD_40042_Gardist,"WP_ATTACKNC10"); //3g
	Wld_InsertNpc				(GRD_40043_Gardist,"WP_ATTACKNC11"); //3g
...
INSTANCE DIA_PC_Fighter_HELLO_Pz (C_INFO)
{
   npc          = PC_Fighter;
   nr           = 1;
   condition    = DIA_PC_Fighter_HELLO_Pz_Condition;
   information  = DIA_PC_Fighter_HELLO_Pz_Info;
   permanent	= FALSE;
   description	= "Jest pewien problem.";
};

FUNC INT DIA_PC_Fighter_HELLO_Pz_Condition()
{
if (Npc_KnowsInfo(hero, DIA_Foster_GoWMe)) 
{
    return TRUE;
};
};
FUNC VOID DIA_PC_Fighter_HELLO_Pz_Info()
{
    AI_Output (other, self ,"DIA_PC_Fighter_HELLO_Pz_15_01"); //Jest pewien problem.
    AI_Output (self, other ,"DIA_PC_Fighter_HELLO_Pz_03_02"); //Jaki?
    AI_Output (other, self ,"DIA_PC_Fighter_HELLO_Pz_15_03"); //Foster - cz³owiek, który obok mnie stoi wpakowa³ siê w niez³e gówno. Wspó³pracowa³ z rozbójnikami. Potem zerwa³ wspó³pracê ale oni wymyœli sobie nowy cel.
    AI_Output (other, self ,"DIA_PC_Fighter_HELLO_Pz_15_04"); //Postanowili zdobyæ plany fortyfikacji Nowego Obozu i sprzedaæ je Magnatom Gomeza.
    AI_Output (self, other ,"DIA_PC_Fighter_HELLO_Pz_03_05"); //Trzeba przyznaæ, ¿e to cwany plan jak na prostych zbójów. Jak dok³adnie chcieli go zrealizowaæ?
    AI_Output (other, self ,"DIA_PC_Fighter_HELLO_Pz_15_06"); //Foster mia³ okraœæ swojego znajomego - Homera. I zanieœæ pisma rozbójnikom.
    AI_Output (self, other ,"DIA_PC_Fighter_HELLO_Pz_03_07"); //Rzeczywiœcie brzmi jak plan. Ale czego w³aœciwie ode mnie oczekujesz?
    AI_Output (other, self ,"DIA_PC_Fighter_HELLO_Pz_15_08"); //A co gdyby po prostu zabiæ tych zbójów?
    AI_Output (self, other ,"DIA_PC_Fighter_HELLO_Pz_03_09"); //Nie zg³aszaj¹c tego Lee i pozosta³ym?
    AI_Output (other, self ,"DIA_PC_Fighter_HELLO_Pz_15_10"); //Foster pewnie wie gdzie jest kryjówka zbójów. A Lee nie musi wiedzieæ o czymœ czego wkrótce nie bêdzie.
    AI_Output (other, self ,"DIA_PC_Fighter_HELLO_Pz_15_11"); //Foster dorzuci sporo rudy i mo¿emy wyjœæ na swoje. 
    AI_Output (self, other ,"DIA_PC_Fighter_HELLO_Pz_03_12"); //Hmmm dobra walka zawsze w cenie. Zw³aszcza gdy jest solidny motyw.
    AI_Output (self, other ,"DIA_PC_Fighter_HELLO_Pz_03_13"); //Wezmê w tym udzia³. Parê kanalii trafi do piachu. 
    AI_Output (self, other ,"DIA_PC_Fighter_HELLO_Pz_03_14"); //Za³atw od Fostera by zaprowadzi³ nas do swoich dawnych koleszków.
    AI_Output (other, self ,"DIA_PC_Fighter_HELLO_Pz_15_15"); //Dobra.
	AI_Teleport(BAU_924_Foster,"NC_PATH77"); 
	
	B_LogEntry           (PoszkodowaniZbieracze,"Gorn chcia³ wiedzieæ dok³adnie o co chodzi. Wyjaœni³em mu wszystko a on zdaje siê mo¿e pomóc. Muszê za³atwiæ by Foster zaprowadzi³ nas do obozu swoich dawnych kompanów.");
};

//========================================
//-----------------> GOWH_PZ
//========================================
var int FollowInAroundPZ;
INSTANCE DIA_PC_Fighter_GOWH_PZ (C_INFO)
{
   npc          = PC_Fighter;
   nr           = 1;
   condition    = DIA_PC_Fighter_GOWH_PZ_Condition;
   information  = DIA_PC_Fighter_GOWH_PZ_Info;
   permanent	= FALSE;
   Important    = TRUE;
};

FUNC INT DIA_PC_Fighter_GOWH_PZ_Condition()
{
if (Npc_KnowsInfo(hero,DIA_Foster_GO_PZG))
{
    return TRUE;
};
};
FUNC VOID DIA_PC_Fighter_GOWH_PZ_Info()
{
    AI_Output (self, other ,"DIA_PC_Fighter_GOWH_PZ_03_01"); //Czyli wszystko gotowe?
...
FUNC VOID DIA_Gorn_PZGS()
{
	...
	AI_Teleport(BAU_924_Foster,"OW_PATH_039");
	AI_Teleport(hero,"OW_PATH_039");
	
	FollowInAroundPZ = TRUE;
	Wld_InsertNpc 	(ORG_40160_Szef,        "GHOST_OLDBANDIT");
  	Wld_InsertNpc 	(Org_40161_Rozbojnik,   "GHOST_OLDBANDIT");
  	Wld_InsertNpc 	(Org_40159_Rozbojnik,   "GHOST_OLDBANDIT");
	 Npc_ExchangeRoutine(Org_40159_Rozbojnik,"standpz");
};
...
INSTANCE DIA_PC_Fighter_PZ_ENDEG (C_INFO)
{
   npc          = PC_Fighter;
   nr           = 1;
   condition    = DIA_PC_Fighter_PZ_ENDEG_Condition;
   information  = DIA_PC_Fighter_PZ_ENDEG_Info;
   permanent	= FALSE;
   important    = TRUE;
};

FUNC INT DIA_PC_Fighter_PZ_ENDEG_Condition()
{
...
FUNC VOID DIA_PC_Fighter_PZ_ENDEG_Info()
{
    AI_Output (self, other ,"DIA_PC_Fighter_PZ_ENDEG_03_01"); //No i po rozbójnikach.
    AI_Output (other, self ,"DIA_PC_Fighter_PZ_ENDEG_15_02"); //Nie zrobi¹ ju¿ nikomu problemów.
    AI_Output (self, other ,"DIA_PC_Fighter_PZ_ENDEG_03_03"); //Niez³a walka. Ci zbóje przynajmniej zapewnili nam trochê wojenne praktyki.
    AI_Output (other, self ,"DIA_PC_Fighter_PZ_ENDEG_15_04"); //I sporo nerwów Fosterowi. Pogadam z nim. Ty te¿ to zrób bo jest ci winien rudê za pomoc.
