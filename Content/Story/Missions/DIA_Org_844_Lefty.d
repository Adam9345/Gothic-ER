...
FUNC VOID DIA_Lefty_Free_Info()
{
	...
	AI_Output (self, other,"DIA_Lefty_Free_07_04"); //Eee... Bo tak mi siê podoba! Nie chcesz mnie chyba bardziej zdenerwowaæ? Po prostu zg³oœ siê do mnie jutro.
...
INSTANCE DIA_Lefty_Plaga (C_INFO)
{
   npc          = Org_844_Lefty;
   nr           = 2;
...
FUNC VOID DIA_Lefty_Plaga_Info()
{
    AI_Output (other, self ,"DIA_Lefty_Plaga_15_01"); //Ry¿owy Ksi¹¿ê potrzebuje cz³owieka do pilnowania magazynu.
    AI_Output (self, other ,"DIA_Lefty_Plaga_03_02"); //No to rusz dupê i zajmij siê tym.
    AI_Output (other, self ,"DIA_Lefty_Plaga_15_03"); //le mnie zrozumia³eœ. Mia³em na myœli ciebie.
    AI_Output (self, other ,"DIA_Lefty_Plaga_03_04"); //Hahaha dobry ¿art. Nawet palcem nie kiwnê dla takiego pata³acha jak ty.
    AI_Output (other,self ,"DIA_Lefty_Plaga_03_05"); //Zaraz siê przekonamy.
    AI_DrawWeapon (other);
	Org_844_Lefty.attribute[ATR_HITPOINTS] = 220;
	Org_844_Lefty.attribute[ATR_HITPOINTS_MAX] = 220;
	EquipItem (Org_844_Lefty, ItMw_1H_Mace_03_Z);
    AI_Output (self, other ,"DIA_Lefty_Plaga_03_06"); //Mo¿esz tak straszyæ ma³e dzieci ale nie mnie!
    AI_Output (other, self ,"DIA_Lefty_Plaga_15_07"); //Jak sobie chcesz. Dostaniesz po g³owie!
    AI_StopProcessInfos	(self);
    Npc_SetTarget (self, other);
    AI_StartState (self, ZS_ATTACK, 1, "");
...
INSTANCE DIA_Lefty_P_Pajac (C_INFO)
{
   npc          = Org_844_Lefty;
   nr           = 1;
   condition    = DIA_Lefty_P_Pajac_Condition;
   information  = DIA_Lefty_P_Pajac_Info;
   permanent	= FALSE;
   description	= "No i co teraz?";
};

FUNC INT DIA_Lefty_P_Pajac_Condition()
{
if (Npc_KnowsInfo(hero,DIA_Lefty_Plaga))
{
    return TRUE;
};
};
FUNC VOID DIA_Lefty_P_Pajac_Info()
{
    AI_Output (other, self ,"DIA_Lefty_P_Pajac_15_01"); //No i co teraz?
    AI_Output (self, other ,"DIA_Lefty_P_Pajac_03_02"); //Nic. Nie bêdê twoim s³ugusem. Goñ siê!
    AI_Output (other, self ,"DIA_Lefty_P_Pajac_15_03"); //Bezu¿yteczny pajac. W takim razie poszukam gdzie indziej.
    AI_PlayAni (self, "T_SEARCH");
    AI_Output (self, other ,"DIA_Lefty_P_Pajac_03_04"); //ZejdŸ mi z oczu, frajerze.
