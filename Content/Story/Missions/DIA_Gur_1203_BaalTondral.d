...
FUNC VOID DIA_BaalTondral_GiveMeStaff_Weapon()
{
	...
	B_LogEntry          (CH4_MagicalCrystals,"Ok³ama³em Baala Tondrala. Nie odda mi kostura. Muszê wróciæ i porozmawiaæ z Uturielem.");
...
//========================================
var int Zdrajca_S;
INSTANCE DIA_BaalTondral_Zdrajca (C_INFO)
{
   npc          = Gur_1203_BaalTondral;
   nr           = 1;
   condition    = DIA_BaalTondral_Zdrajca_Condition;
   information  = DIA_BaalTondral_Zdrajca_Info;
   permanent	= FALSE;
   important    = TRUE;
};

FUNC INT DIA_BaalTondral_Zdrajca_Condition()
{
...
FUNC VOID DIA_BaalTondral_Zdrajca_Info()
{
    AI_Output (self, other ,"DIA_BaalTondral_Zdrajca_03_01"); //PodejdŸ stra¿niku. Powinniœmy pomówiæ.
    AI_Output (other, self ,"DIA_BaalTondral_Zdrajca_15_02"); //Tak Baalu?
    AI_Output (self, other ,"DIA_BaalTondral_Zdrajca_03_03"); //Tu¿ przed twoim przybyciem Cor Calom nakaza³ mi za³atwienie jego niedokoñczonej sprawy. 
    AI_Output (self, other ,"DIA_BaalTondral_Zdrajca_03_04"); //Mianowicie, stosunkowo niedawno z obozu zbieg³ jeden z najlepszych Nowicjuszy imieniem Gaston. Skrad³ on pewne bezcenne prastare tablice. 
    AI_Output (self, other ,"DIA_BaalTondral_Zdrajca_03_05"); //Wol¹ Caloma jest odzyskanie tablic i zg³adzenia uciekiniera.
    AI_Output (other, self ,"DIA_BaalTondral_Zdrajca_15_06"); //Ile by³o tych tablic i gdzie nale¿y go szukaæ?
    AI_Output (self, other ,"DIA_BaalTondral_Zdrajca_03_07"); //Skradziono 5 tabliczek, jedyne co wiadomo to fakt, ¿e jego kuzynem by³ niejaki Gilbert i byæ mo¿e to do niego uda³ siê ten zdrajca.
    AI_Output (self, other ,"DIA_BaalTondral_Zdrajca_03_08"); //Zajmiesz siê nim?
    AI_Output (other, self ,"DIA_BaalTondral_Zdrajca_15_09"); //Mam go zabiæ? 
    AI_Output (self, other ,"DIA_BaalTondral_Zdrajca_03_10"); //Zas³u¿y³ na to. Zdradzi³ swój obóz i wspó³braci.	
	AI_Output (other, self ,"DIA_BaalTondral_Zdrajca_15_11"); //Zajmê siê nim...
    AI_Output (self, other ,"DIA_BaalTondral_Zdrajca_03_12"); //Oczywiœcie dobrze by³oby gdybyœ odnalaz³ tak¿e te zaginione tablice.
    AI_Output (other, self ,"DIA_BaalTondral_Zdrajca_15_13"); //Postaram siê.
    AI_Output (self, other ,"DIA_BaalTondral_Zdrajca_03_14"); //Dobrze, zatem mogê powróciæ do swoich zajêæ.
    AI_Output (self, other ,"DIA_BaalTondral_Zdrajca_03_15"); //Powodzenia.
	Zdrajca_S = LOG_RUNNING;
    Log_CreateTopic		(PSI_Zdrajca,	LOG_MISSION);
	Log_SetTopicStatus	(PSI_Zdrajca,	LOG_RUNNING);
	B_LogEntry          (PSI_Zdrajca,"Baal Tondral poinformowa³ mnie, ¿e niejaki Gaston skrad³ pewne prastare tablice. Cor Calom chce ¿eby go pochwyciæ i zabiæ, odzyskuj¹c przy tym tablice. Muszê odnaleŸæ Gastona. Wiadomo ¿e jego kuzynem by³ niejaki Gilbert. I to wydaje siê byæ przydatn¹ informacj¹...");
    AI_StopProcessInfos	(self);
};
...
INSTANCE DIA_BaalTondral_Ghaston_Live (C_INFO)
{
   npc          = Gur_1203_BaalTondral;
   nr           = 1;
   condition    = DIA_BaalTondral_Ghaston_Live_Condition;
   information  = DIA_BaalTondral_Ghaston_Live_Info;
   permanent	= FALSE;
   description    = "Odnalaz³em Gastona.";
};

FUNC INT DIA_BaalTondral_Ghaston_Live_Condition()
{
    if (Npc_KnowsInfo (hero, DIA_Arnold_NoDeath))
...
FUNC VOID DIA_BaalTondral_Ghaston_Live_Info()
{
    AI_Output (other, self ,"DIA_BaalTondral_Ghaston_Live_15_01"); //Odnalaz³em Gastona.
    AI_Output (self, other ,"DIA_BaalTondral_Ghaston_Live_03_02"); //I zabi³eœ go...
    AI_Output (other, self ,"DIA_BaalTondral_Ghaston_Live_15_03"); //Nie. 
    AI_Output (self, other ,"DIA_BaalTondral_Ghaston_Live_03_04"); //Co takiego?
    AI_Output (other, self ,"DIA_BaalTondral_Ghaston_Live_15_05"); //Pos³uchaj Baalu. Gaston odkry³, ¿e na tych skradzionych tablicach s¹ informacjê o tym, ¿e Œni¹cy jest demonem. Oraz dziwne zachowanie Cor Caloma.
    AI_Output (self, other ,"DIA_BaalTondral_Ghaston_Live_03_06"); //Co?! 
    AI_Output (other, self ,"DIA_BaalTondral_Ghaston_Live_15_07"); //Tak. Oprócz tego wedle tego proroctwa bractwo upadnie a jeden z jego przywódców oka¿ê siê zdrajc¹. 
    AI_Output (self, other ,"DIA_BaalTondral_Ghaston_Live_03_08"); //A ja kaza³em go zabiæ... Có¿, co do Caloma to bêdê mia³ na niego oko i te jego eksperymenty...
    AI_Output (other, self ,"DIA_BaalTondral_Ghaston_Live_15_09"); //Nie wiedzia³eœ.
    AI_Output (self, other ,"DIA_BaalTondral_Ghaston_Live_03_10"); //Gaston jest bezpieczny?  
	AI_Output (other, self ,"DIA_BaalTondral_Ghaston_Live_15_11"); //Tak do³¹czy³ do myœliwych i przyj¹³ imiê Arnold.
    AI_Output (self, other ,"DIA_BaalTondral_Ghaston_Live_03_12"); //Dobrze. Zas³u¿y³ na spokój.
    AI_Output (other, self ,"DIA_BaalTondral_Ghaston_Live_15_13"); //Pomówiê z nim.
    AI_Output (self, other ,"DIA_BaalTondral_Ghaston_Live_03_14"); //A co z tablicami?
    AI_Output (other, self ,"DIA_BaalTondral_Ghaston_Live_15_15"); //S¹ dalej u niego.
    AI_Output (self, other ,"DIA_BaalTondral_Ghaston_Live_03_16"); //I niech zostan¹. Tam bêd¹ bezpieczne.
    AI_Output (self, other ,"DIA_BaalTondral_Ghaston_Live_03_17"); //Do zobaczenia!
    AI_Output (other, self ,"DIA_BaalTondral_Ghaston_Live_15_18"); //Trzymaj siê.
	B_GiveXP (450);
    B_LogEntry               (PSI_Zdrajca,"Baal Tondral jest ze mnie zadowolony. Przeka¿ê Arnoldowi ¿e mo¿e zachowaæ tablice, oraz ¿e jest ju¿ bezpieczny.");
...
INSTANCE DIA_BaalTondral_Ghaston_Dead (C_INFO)
{
   npc          = Gur_1203_BaalTondral;
   nr           = 1;
   condition    = DIA_BaalTondral_Ghaston_Dead_Condition;
   information  = DIA_BaalTondral_Ghaston_Dead_Info;
   permanent	= FALSE;
   description    = "Zdrajca nie ¿yje.";
};

FUNC INT DIA_BaalTondral_Ghaston_Dead_Condition()
{
    if  (GhastonIsDead == TRUE)
    && (Npc_HasItems (other, ItMi_Tondrall_Slab) >=5)
    {
    return TRUE;
    };
};
...
FUNC VOID DIA_BaalTondral_Ghaston_Dead_Info()
{
    AI_Output (other, self ,"DIA_BaalTondral_Ghaston_Dead_15_01"); //Zdrajca nie ¿yje.
    AI_Output (self, other ,"DIA_BaalTondral_Ghaston_Dead_03_02"); //Œwietnie. A co z tablicami.
    AI_Output (other, self ,"DIA_BaalTondral_Ghaston_Dead_15_03"); //Oto one.
	B_GiveInvItems (other,self, ItMi_Tondrall_Slab, 5);
    AI_Output (self, other ,"DIA_BaalTondral_Ghaston_Dead_03_04"); //Znakomicie siê spisa³eœ.
    AI_Output (self, other ,"DIA_BaalTondral_Ghaston_Dead_03_06"); //Oto twoja nagroda.
    AI_Output (other, self ,"DIA_BaalTondral_Ghaston_Dead_15_07"); //Dziêkuje.
    AI_Output (self, other ,"DIA_BaalTondral_Ghaston_Dead_03_08"); //Cieszê siê, ¿e do nas nale¿ysz. Jesteœ lojalnym i twardym wojownikiem.
    AI_Output (other, self ,"DIA_BaalTondral_Ghaston_Dead_15_09"); //Dziêkuje za s³owa uznania.
    AI_Output (self, other ,"DIA_BaalTondral_Ghaston_Dead_03_10"); //Trzymaj siê.
	AI_Output (other, self ,"DIA_BaalTondral_Ghaston_Dead_15_11"); //Do zobaczenia.   
    B_LogEntry               (PSI_Zdrajca,"Baal Tondral jest ze mnie zadowolony. Wzi¹³ kamienne tablice I wyp³aci³ mi ca³kiem godziwe honorarium.");	
