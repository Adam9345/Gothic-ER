...
FUNC VOID DIA_Firn_TEACH_Info()
{
	...
	{
	Info_AddChoice		(DIA_Firn_TEACH, "Usuwanie miêsa topielców (150 bry³ek rudy, 1 PN)", DIA_Firn_TEACH_LurkerMeat);
	};
	if Knows_GetMoleratW == false
	{
    Info_AddChoice		(DIA_Firn_TEACH, "Usuwanie w¹troby kretoszczurów (50 bry³ek rudy, 1 PN)", DIA_Firn_TEACH_MoleratW);
	};
};
...
func void DIA_Firn_TEACH_Futra ()
{
    ...
    if (Npc_HasItems (hero, ItMiNugget)>=50) && (Knows_GetFur == FALSE)
    {
		if (hero.lp >= 1)
		{
...
func void DIA_Firn_TEACH_Gady ()
{
    ...
    if (Npc_HasItems (hero, ItMiNugget)>=50) && (Knows_GetHide == FALSE)
    {
		if (hero.lp >= 1)
		{
		Knows_GetHide = TRUE;	
...
FUNC VOID DIA_Firn_TEACH_LurkerMeat()
{
    AI_Output (other, self ,"DIA_Firn_TEACH_LurkerMeat_15_01"); //Chcê nauczyæ siê usuwaæ miêso topielców.
    if (Npc_HasItems (hero, ItMiNugget)>=150) && (Knows_GetLurkerMeat == FALSE)
    {
    if (hero.lp >= 1)
    {
        AI_Output (self, other ,"DIA_Firn_TEACH_LurkerMeat_03_02"); //Miêso trzeba wyci¹æ z górnych czêœci u nóg.
        AI_Output (self, other ,"DIA_Firn_TEACH_LurkerMeat_03_03"); //Nogi topielca s¹ silnie umiêœnione. 
        AI_Output (self, other ,"DIA_Firn_TEACH_LurkerMeat_03_04"); //Nale¿y powoli odcinaæ miêso od koœci. 
        AI_Output (self, other ,"DIA_Firn_TEACH_LurkerMeat_03_05"); //To nie jest zbyt trudne. Dasz radê.
    ...
    {
	 AI_Output (self, other ,"DIA_Firn_TEACH_LurkerMeat_03_06"); //Brakuje ci doœwiadczenia!
    ...
    {
	   AI_Output (self, other ,"DIA_Firn_TEACH_LurkerMeat_03_07"); //Nie masz doœæ rudy!
    };

};

FUNC VOID DIA_Firn_TEACH_MoleratW()
{
    if (Npc_HasItems (hero, ItMiNugget)>=50) && (Knows_GetMoleratW == FALSE)
    {
    if (hero.lp >= 1)
    {
        AI_Output (self, other ,"DIA_Firn_TEACH_MoleratW_03_02"); //W¹troba kretoszczura jest umieszczona doœæ wysoko.
        AI_Output (self, other ,"DIA_Firn_TEACH_MoleratW_03_03"); //Musisz obróciæ zwierza na grzbiet i rozci¹æ jego brzuch.
        AI_Output (self, other ,"DIA_Firn_TEACH_MoleratW_03_04"); //Dalej chyba sobie poradzisz.
    ...
    {
	 AI_Output (self, other ,"DIA_Firn_TEACH_MoleratW_03_06"); //Brakuje ci doœwiadczenia!
	 PrintScreen	("Za ma³o punktów umiejêtnoœci!", -1,-1,"FONT_NEW_10_RED.TGA",2);
    };
    ...
    {
	   AI_Output (self, other ,"DIA_Firn_TEACH_MoleratW_03_07"); //Nie masz doœæ rudy!
    };
};
//========================================
//-----------------> HELLO6
...
FUNC VOID DIA_Firn_PorwanieInicjacja_Info()
{
    ...
    if (MIS_PNA == LOG_RUNNING)
	{
	B_LogEntry                     (PolowanieNaAligatory,"Alex zosta³ porwany przez stra¿ników Gomeza, w¹tpiê aby chcia³ jeszcze robiæ interesy ze Starym Obozem.");
	Log_SetTopicStatus             (PolowanieNaAligatory, LOG_FAILED);
	MIS_PNA = LOG_FAILED;
	 Npc_ExchangeRoutine(NON_5602_Stanley,"Start");
...
instance dia_firn_pickpocket(c_info) {
    npc = non_5601_firn;
    nr = 900;
    condition = dia_firn_pickpocket_condition;
    information = dia_firn_pickpocket_info;
    permanent = 1;
    description = pickpocket_final;
};

func int dia_firn_pickpocket_condition() {
	e_beklauen(baseThfChanceORG, 30);
};

func void dia_firn_pickpocket_info() {
	b_steal_message();
	info_clearchoices(dia_firn_pickpocket);
	info_addchoice(dia_firn_pickpocket, dialog_back, dia_firn_pickpocket_back);
	info_addchoice(dia_firn_pickpocket, dialog_pickpocket, dia_firn_pickpocket_doit);
};

func void dia_firn_pickpocket_doit() {
    d_beklauen();
    info_clearchoices(dia_firn_pickpocket);
};

func void dia_firn_pickpocket_back() {
    info_clearchoices(dia_firn_pickpocket);
};