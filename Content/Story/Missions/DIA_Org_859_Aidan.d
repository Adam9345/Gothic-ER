...
{
    AI_Output (other, self ,"DIA_Aidan_SZPIEGOWANIE_15_01"); //Czeœæ. 
...
FUNC VOID DIA_Aidan_Aidan_Quest_Key_Info()
{
    ...
    AI_Output (self, other ,"DIA_Aidan_Aidan_Quest_Key_03_02"); //O bogowie, na œmieræ zapomnia³em. WeŸ ten klucz do skrzyni.
 //   AI_Output (other, self ,"DIA_Aidan_Aidan_Quest_Key_15_03"); //Nie, natkn¹³em siê za to na solidnie zabezpieczon¹ skrzyniê. Bez klucza nic nie poradzê.
  //  AI_Output (self, other ,"DIA_Aidan_Aidan_Quest_Key_03_04"); //Ma³o brakowa³o
...
INSTANCE DIA_Aidan_TMQ_BOSS (C_INFO)
{
   npc          = ORG_859_Aidan;
   nr           = 1;
   condition    = DIA_Aidan_TMQ_BOSS_Condition;
   information  = DIA_Aidan_TMQ_BOSS_Info;
   permanent	= FALSE;
   Important    = TRUE;
};

FUNC INT DIA_Aidan_TMQ_BOSS_Condition()
{
if (Npc_KnowsInfo(hero,DIA_Rocky_TMQ_PATH))
{
    return TRUE;
};
};
FUNC VOID DIA_Aidan_TMQ_BOSS_Info()
{
    AI_Output (self, other ,"DIA_Aidan_TMQ_BOSS_03_01"); //O ludzie Quentina! Nareszcie jesteœcie!
 //  AI_Output (other, self ,"DIA_Aidan_TMQ_BOSS_15_02"); //Zgadza siê.
    AI_Output (self, other ,"DIA_Aidan_TMQ_BOSS_03_03"); //Wasz szef wpad³ w ³apy Stra¿ników. Broni³ siê jak lew ale nie da³ rady.
    AI_Output (self, other ,"DIA_Aidan_TMQ_BOSS_03_04"); //Widzia³em to, bo w³aœnie wraca³em z polowania.
    AI_Output (other, self ,"DIA_Aidan_TMQ_BOSS_15_05"); //Gdzie on teraz jest?
    AI_Output (self, other ,"DIA_Aidan_TMQ_BOSS_03_06"); //Zabrali go w stronê starej krypty, prawdopodobnie do tamtejszej ledwo stoj¹cej szopy która znajduje siê nieopodal.
    AI_Output (other, self ,"DIA_Aidan_TMQ_BOSS_15_07"); //Dziêki za informacje, mam nadziejê ¿e jeszcze ¿yje...
    AI_Output (self, other ,"DIA_Aidan_TMQ_BOSS_03_08"); //Drobiazg. Nie lubiê Stra¿y i Gomeza.
	
	
    Info_Clearchoices (DIA_Aidan_TMQ_BOSS);
	Info_Addchoice (DIA_Aidan_TMQ_BOSS,DIALOG_BACK,DIA_Aidan_CampInfo_BACK); 
	Info_Addchoice (DIA_Aidan_TMQ_BOSS,"Ilu by³o stra¿ników?",DIA_Aidan_CampInfo_GIL);
	Info_Addchoice (DIA_Aidan_TMQ_BOSS,"Czemu zabrali go akurat w tamto mjejsce?",DIA_Aidan_CampInfo_SLEEPER);
	Info_Addchoice (DIA_Aidan_TMQ_BOSS,"Masz dla mnie jakieœ dobre rady?",DIA_Aidan_CampInfo_HERB);
	
	Wld_InsertNpc				(Grd_40075_Gardist,"OW_PATH_WARAN06_SPAWN01"); 
	Wld_InsertNpc				(Grd_40076_Gardist,"OW_PATH_WARAN06_SPAWN01");
	Wld_InsertNpc				(Grd_40077_Gardist,"OW_PATH_WARAN06_SPAWN01");
	Wld_InsertNpc				(Grd_40078_Gardist,"OW_PATH_WARAN06_SPAWN01");
	B_LogEntry                  (TajnaMisjaQuentina,"Spotkaliœmy Aidana. Twierdzi on, ¿e Stra¿nicy pochwycili Quentina a nastêpnie zabrali go prawdopodobnie do starej szopy która znajduje siê po drodze do starej krypty.");
    //AI_StopProcessInfos	(self);
};

func void DIA_Aidan_CampInfo_BACK()
{
	Info_Clearchoices (DIA_Aidan_TMQ_BOSS);
};

func void DIA_Aidan_CampInfo_GIL()
{
	AI_Output (other, self,"DIA_Aidan_CampInfo_GIL_15_00"); //Ilu by³o stra¿ników?
	AI_Output (self, other,"DIA_Aidan_CampInfo_GIL_05_01"); //Wydaje mi siê ¿e oko³o szeœciorga, to dzia³o siê tak szybko...
	AI_Output (self, other,"DIA_Aidan_CampInfo_GIL_05_02"); //Wiem jednak ¿e to nie byli jacyœ s³abeusze, znaczna czêœæ z nich by³a uzbrojona po zêby i nosili ciê¿kie zbroje.
	AI_Output (other,self ,"DIA_Aidan_CampInfo_GIL_15_03"); //Interesuj¹ce...
	B_GiveXP(30);
};
func void DIA_Aidan_CampInfo_SLEEPER()
{
	AI_Output (other, self,"DIA_Aidan_CampInfo_SLEEPER_15_00"); //Czemu zabrali go akurat w tamto miejsce?
	AI_Output (self, other,"DIA_Aidan_CampInfo_SLEEPER_05_01"); //A ja ci to wiem, mo¿e po prostu im siê tak uwidzia³o...
	AI_Output (self, other,"DIA_Aidan_CampInfo_SLEEPER_05_02"); //Chocia¿ nie zdziwi³ bym siê gdyby mieli tam jakiœ tymczasowy obóz, z tamtego miejsca dosyæ dobrze widaæ bramê do nowego obozu.
	AI_Output (self, other,"DIA_Aidan_CampInfo_SLEEPER_05_03"); //Wiesz co mam na myœli...
	B_GiveXP(30);
};
func void DIA_Aidan_CampInfo_HERB()
{
	AI_Output (other, self,"DIA_Aidan_CampInfo_HERB_15_00"); //Masz dla mnie jakieœ dobre rady?
	AI_Output (self, other,"DIA_Aidan_CampInfo_HERB_05_01"); //Lepiej rusz ty³ek jak chcesz uratowaæ swojego szefa, i nie zapomnij siê dobrze przygotowaæ do walki z tymi osi³kami...
	AI_Output (self, other,"DIA_Aidan_CampInfo_HERB_05_02"); //WeŸ jeszcze te miksturê, dosta³em j¹ kiedyœ od Cronosa za dostarczenie rzadkich sk³adników.
