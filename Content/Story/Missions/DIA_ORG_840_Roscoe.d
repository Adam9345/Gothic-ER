...
//--------------------------------------Zmiana rutyn Sharky’ego,Ciphera i Butcha------------------------------------------------

func void rutynySCB()
{ 

    Konwoj_Day  = Wld_GetDay(); //<=(Wld_GetDay()); 
    ORG_843_Sharky.aivar[AIV_PARTYMEMBER] = TRUE;
    Npc_ExchangeRoutine (ORG_843_Sharky,"konwoj");
	ORG_851_Butch.aivar[AIV_PARTYMEMBER] = TRUE;
    Npc_ExchangeRoutine (ORG_851_Butch,"konwoj");
	ORG_873_Cipher.aivar[AIV_PARTYMEMBER] = TRUE;
    Npc_ExchangeRoutine (ORG_873_Cipher,"konwoj");	
	Npc_SetTrueGuild	(VLK_40044_Inzynier,GIL_GOBBO);
...
INSTANCE DIA_Roscoe_Konwoj (C_INFO)
{
   npc          = Org_840_Roscoe;
   nr           = 1;
   condition    = DIA_Roscoe_Konwoj_Condition;
   information  = DIA_Roscoe_Konwoj_Info;
   permanent	= FALSE;
   description	= "Lares mówi, ¿e masz mi powiedzieæ szczegó³y na temat ataku...";
};

FUNC INT DIA_Roscoe_Konwoj_Condition()
{
    if (Npc_KnowsInfo (hero, DIA_Lares_Konwoj))
    {
    return TRUE;
    };
};
...
FUNC VOID DIA_Roscoe_Konwoj_Info()
{
    AI_Output (other, self ,"DIA_Roscoe_Konwoj_15_01");  //Lares mówi, ¿e masz mi powiedzieæ szczegó³y na temat ataku na konwój prowadz¹cy in¿yniera Gomeza do Starej Kopalni.
	ZwSk_Attack = TRUE;
    AI_Output (self, other ,"DIA_Roscoe_Konwoj_03_02");  //Kwadrans przed wyruszeniem konwoju macie spotkaæ siê w lesie pomiêdzy Starym Obozem a Kopalni¹.
    AI_Output (other, self ,"DIA_Roscoe_Konwoj_15_03");  //My to znaczy kto?
    AI_Output (self, other ,"DIA_Roscoe_Konwoj_03_04");  //Ty i reszta ch³opaków. Chodzi o Sharky’ego, Ciphera i Butcha.
    AI_Output (other, self ,"DIA_Roscoe_Konwoj_15_05");  //A wiêc ju¿ chyba wszystko jasne.
    
    B_LogEntry                     (Zawirowania,"Lares poleci³ mi rozmówiæ siê z Roscoe w sprawie napadu na konwój z in¿ynierem. Ten mówi, ¿e mam siê udaæ do lasu nieopodal Starej Kopalni. Mam tam siê zasadziæ z paroma Szkodnikami. In¿ynier ma paœæ martwy pod naszymi ciosami.");
