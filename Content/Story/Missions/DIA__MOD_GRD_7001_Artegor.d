...
{
    if (Npc_GetTrueGuild(hero) == GIL_BAU) && (GoToOM == false) && (Npc_HasItems (other, ItWr_CorCToArtegor) >=1)
    {
    return TRUE;
    };
};
...
FUNC VOID DIA_Artegor_BANDYTA_Info()
{
    ...
    AI_Output (other, self ,"DIA_Artegor_BANDYTA_15_03"); //Mam sprawê do za³atwienia w kopalni, oto pismo od Cor Caloma. Pozwól mi przejœæ. 
	B_GiveInvItems (other,self, ItWr_CorCToArtegor, 1);
	Npc_RemoveInvItems(self,ItWr_CorCToArtegor,1);
	B_UseFakeScroll( );
    AI_Output (self, other ,"DIA_Artegor_BANDYTA_03_04"); //Rzeczywiœcie ale pamiêtaj ¿e jeœli tylko zaczniesz sprawiaæ k³opoty... oj po¿a³ujesz. 
    AI_Output (self, other ,"DIA_Artegor_BANDYTA_03_05"); //I nie mogê siê doczekaæ, a¿ znajdziemy ten wasz zapchlony Obóz. 
    AI_Output (self, other ,"DIA_Artegor_BANDYTA_03_06"); //NO DALEJ! W³aŸ!
    AI_StopProcessInfos	(self);
	GoToOM = true;
};
...
FUNC VOID DIA_Artegor_QuestIan_Succes_Info()
{
	...
	AI_Teleport(GRD_7007_Mirzo,"MIRZO_PAKA");
...
FUNC VOID DIA_Artegor_Cheat_Buddlers ()
{
	...
	AI_Output (self, other ,"DIA_Artegor_Cheat_Buddlers_03_04"); //A ¿ebyœ wiedzia³, ¿e zaszczyt. 
	Info_ClearChoices		(DIA_Artegor_Cheat);
...
INSTANCE DIA_Artegor_PP_WHAT (C_INFO)
{
   npc          = GRD_7001_Artegor;
   nr           = 1;
   condition    = DIA_Artegor_PP_WHAT_Condition;
   information  = DIA_Artegor_PP_WHAT_Info;
   permanent	= FALSE;
   Important    = TRUE;
};

FUNC INT DIA_Artegor_PP_WHAT_Condition()
{
...
FUNC VOID DIA_Artegor_PP_WHAT_Info()
{
    AI_Output (self, other ,"DIA_Artegor_PP_WHAT_03_01"); //A co to tutaj lezie? To jakiœ ¿art! Co ty masz na sobie?
    AI_Output (other, self ,"DIA_Artegor_PP_WHAT_15_02"); //Nie przeczê, i¿ mam doœæ... specyficzny strój.
    AI_Output (self, other ,"DIA_Artegor_PP_WHAT_03_03"); //Co to stary Diego schodzi na psy i przyjmuje w sk³ad Cieni byle kogo?
    AI_Output (self, other ,"DIA_Artegor_PP_WHAT_03_04"); //Wygl¹dasz jak 1000 nieszczêœæ. Coœ ty wyci¹gn¹³ to trêdowatemu zêbaczowi z gard³a?
    AI_Output (other, self ,"DIA_Artegor_PP_WHAT_15_05"); //Nale¿ê do Cieni. Po prostu prze¿y³em parê niemi³ych przygód w interesie Starego Obozu.
    AI_Output (self, other ,"DIA_Artegor_PP_WHAT_03_06"); //Jaki wyszczekany piesek... Niez³y z ciebie komediant szczylu.
    AI_Output (self, other ,"DIA_Artegor_PP_WHAT_03_07"); //W³aŸ. Tylko siê poœpiesz bo na widok twojego pancerza chcê mi siê rzygaæ.
	B_LogEntry               (PrawoPascala,"Stra¿nik Artegor by³ doœæ sceptyczyny patrz¹c na stan mojej zrujnowanej zbroi ale wpuœci³ mnie do œrodka. Teraz pora odnaleŸæ Garpa.");
	GoToOM = true;
    AI_StopProcessInfos	(self);
};
