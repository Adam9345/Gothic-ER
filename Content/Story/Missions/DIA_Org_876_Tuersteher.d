...
var int HeroBelongsOC;
//========================================
//-----------------> BELONGS_OC
//========================================

INSTANCE DIA_Tuersteher_BELONGS_OC (C_INFO)
{
   npc          = Org_876_Tuersteher;
   nr           = 1;
   condition    = DIA_Tuersteher_BELONGS_OC_Condition;
   information  = DIA_Tuersteher_BELONGS_OC_Info;
   permanent	= FALSE;
   Important    = TRUE;
};

FUNC INT DIA_Tuersteher_BELONGS_OC_Condition()
{
...
FUNC VOID DIA_Tuersteher_BELONGS_OC_Info()
{
    AI_Output (self, other ,"DIA_Tuersteher_BELONGS_OC_03_01"); //Hej! Chyba nie myœla³eœ sobie ¿e mo¿esz od tak tu sobie wejœæ?
    AI_Output (other, self ,"DIA_Tuersteher_BELONGS_OC_15_02"); //Masz jakiœ problem?
    AI_Output (self, other ,"DIA_Tuersteher_BELONGS_OC_03_03"); //Tak, cz³onkowie starego obozu za wejœcie p³ac¹ dodatkowo 100 bry³ek rudy.
    AI_Output (other, self ,"DIA_Tuersteher_BELONGS_OC_15_04"); //Niech cie szlag!
	HeroBelongsOC = TRUE;
	self.aivar[AIV_PASSGATE] = FALSE;
};
...
INSTANCE DIA_Tuersteher_OC_PAY (C_INFO)
{
   npc          = Org_876_Tuersteher;
   nr           = 2;
   condition    = DIA_Tuersteher_OC_PAY_Condition;
   information  = DIA_Tuersteher_OC_PAY_Info;
   permanent	= FALSE;
   description	= "Masz te cholerne bry³ki.";
};

FUNC INT DIA_Tuersteher_OC_PAY_Condition()
{
 if (Npc_HasItems (other, ItMiNugget) >=100)
 && (Npc_KnowsInfo(hero,DIA_Tuersteher_BELONGS_OC))
{
    return TRUE;
};
};
FUNC VOID DIA_Tuersteher_OC_PAY_Info()
{
    AI_Output (other, self ,"DIA_Tuersteher_OC_PAY_15_01"); //Masz te cholerne bry³ki.
    AI_Output (self, other ,"DIA_Tuersteher_OC_PAY_03_02"); //Œwietnie, zapraszam do œrodka hehe.
	B_GiveInvItems (other,self, itminugget, 100);
	self.aivar[AIV_PASSGATE] = TRUE;
};
