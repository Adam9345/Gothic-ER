...
FUNC VOID DIA_Pacho_ShadowEmil_Info()
{
    ...
    AI_Output (self, other ,"DIA_Pacho_ShadowEmil_03_07"); //Ku mojemu zdziwieniu, ni st¹d ni zow¹d Bandyta rzuci³ siê na Emila i poder¿n¹³ mu gard³o.
...
FUNC INT DIA_Pacho_IhabeFood_Condition()
{
    ...
    && (MIS_CourierFireMage == LOG_RUNNING)
    && (Npc_HasItems (other, ItFoBeer) >=5) //piwo
    && (Npc_HasItems (other, ItFoLoaf) >=3) //chleb
    && (Npc_HasItems (other, ItFo_mutton_01) >=2) //szynka
    && (Npc_HasItems (other, ItFoMutton) >=5) //pieczone miêso
    && (Npc_HasItems (other, ItFoApple) >=2) //jab³ko
    && (Npc_HasItems (other, ItFoWine) >=1) //wino
    {
    return TRUE;
    };
};
