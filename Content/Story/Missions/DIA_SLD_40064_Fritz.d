...
FUNC VOID DIA_Fritz_GWM_Info()
{
    ...
    AI_Output (self, other ,"DIA_Fritz_GWM_03_02"); //Tak. Zgodzi³em siê wyruszyæ z tob¹ by zg³adziæ to orkowe œcierwo.
...
FUNC VOID DIA_Fritz_GWM_GRD_Dead_Info()
{
    ...
    AI_Output (self, other ,"DIA_Fritz_GWM_GRD_Dead_03_01"); //No i to by by³o na tyle z tym patrolem Stra¿y.
...
FUNC VOID DIA_Fritz_HELLO1_Info()
{
    ...
    AI_Output (self, other ,"DIA_Fritz_HELLO1_03_04"); //Co?! O bogowie, no to koniec ze mn¹.
    ...
    AI_Output (self, other ,"DIA_Fritz_HELLO1_03_08"); //Myœla³em, ¿e p³uca wypluje od kaszlu. (kaszlê) Ja umieram, tak stwierdzili Magowie, twierdz¹c, tylko z zió³ rosn¹cych w obozie Sekty mo¿na uwarzyæ wywar mog¹cy mi pomóc.
    AI_Output (other, self ,"DIA_Fritz_HELLO1_15_09"); //Móg³bym udaæ siê do Sekty i zapytaæ o twój eliksir.
    AI_Output (self, other ,"DIA_Fritz_HELLO1_03_10"); //Zrobi³byœ to dla mnie? Pomów proszê z Cainem. Niech przyrz¹dzi jeszcze jedn¹ porcjê eliksiru.
    AI_Output (other, self ,"DIA_Fritz_HELLO1_15_11"); //Tak zrobiê. Wyruszam natychmiast.
    AI_Output (self, other ,"DIA_Fritz_HELLO1_03_12"); //Wybacz, ale z trudem siê poruszam, wiêc nie da³bym z Tob¹ pójœæ, kawa³ka drogi, tylko bym ciê spowalnia³.
...
FUNC VOID DIA_Fritz_GR_END_Info()
{
    ...
    AI_StandUP(self);
    AI_Output (other, self ,"DIA_Fritz_GR_END_15_01"); //Masz. O to twoje lekarstwo.
    AI_Output (self, other ,"DIA_Fritz_GR_END_03_02"); //Dziêkujê. Ale capi! Nie gorzej jednak ni¿ tamte truj¹ce opary alchemiczne Magów. 
    ...
    AI_Output (self, other ,"DIA_Fritz_GR_END_03_04"); //To by³o paskudne, ale ju¿ czuje jak rozgrzewa moje wnêtrznoœci.
    ...
    AI_Output (self, other ,"DIA_Fritz_GR_END_03_09"); //W nagrodê weŸ tê rudê.
...
FUNC VOID DIA_Fritz_HI_FRITZ_Info()
{
    ...
    AI_Output (self, other ,"DIA_Fritz_HI_FRITZ_03_04"); //£apiê, m³ody.
    AI_Output (self, other ,"DIA_Fritz_HI_FRITZ_03_05"); //wkrótce relacje ze Starym Obozem pogorsz¹ siê. Zapamiêtaj moje s³owa. Czujê to w koœciach.
    AI_Output (other, self ,"DIA_Fritz_HI_FRITZ_15_06"); //Sk¹d ta pewnoœæ?
    AI_Output (self, other ,"DIA_Fritz_HI_FRITZ_03_07"); //Mam wiele znajomoœci, wiêc i sporo siê dowiaduje. Niebawem bêdzie bardzo ciekawie. Ja, Fritz, ci to mówiê!
...
instance dia_fritz_pickpocket(c_info) {
    npc = sld_40064_fritz;
    nr = 900;
    condition = dia_fritz_pickpocket_condition;
    information = dia_fritz_pickpocket_info;
    permanent = 1;
    description = pickpocket_final;
};

func int dia_fritz_pickpocket_condition() {
	e_beklauen(baseThfChanceSLD, 35);
};

func void dia_fritz_pickpocket_info() {
	b_steal_message();
	info_clearchoices(dia_fritz_pickpocket);
	info_addchoice(dia_fritz_pickpocket, dialog_back, dia_fritz_pickpocket_back);
	info_addchoice(dia_fritz_pickpocket, dialog_pickpocket, dia_fritz_pickpocket_doit);
};

func void dia_fritz_pickpocket_doit() {
    d_beklauen();
    info_clearchoices(dia_fritz_pickpocket);
};

func void dia_fritz_pickpocket_back() {
    info_clearchoices(dia_fritz_pickpocket);
};