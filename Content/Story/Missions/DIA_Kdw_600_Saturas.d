...
//	SUBMOD
//***************************************************************************
instance Info_Saturas_Fire_Varans (C_INFO)
{
	npc			= KDW_600_Saturas;
	condition	= Info_Saturas_Fire_Varans_Condition;
	information	= Info_Saturas_Fire_Varans_Info;
	permanent	= 0;
	IMPORTANT   = TRUE;
...
FUNC VOID Info_Saturas_Fire_Varans_Info()
{
	AI_Output			(self, other,"Info_Saturas_Fire_Varans_14_00"); //Musimy pomówiæ. Sprawa jest pilna!
	AI_Output			(other, self,"Info_Saturas_Fire_Varans_15_01"); //O co chodzi.
	AI_Output			(self, other,"Info_Saturas_Fire_Varans_14_02"); //O niespodziewane zagro¿enie, które pojawi³o siê dzisiaj.
	AI_Output			(other, self,"Info_Saturas_Fire_Varans_15_03"); //Jakie zagro¿enie?
	AI_Output			(self, other,"Info_Saturas_Fire_Varans_14_04"); //Okazuje siê, ¿e po wymordowaniu Magów Ognia , Magnaci zainteresowali siê pozostawionymi przez nich runami i cennymi zapiskami.
	AI_Output			(self, other,"Info_Saturas_Fire_Varans_14_05"); //Wykorzystali do tego Cienia Jima, który wspó³pracowa³ z Magami i mia³ wgl¹d w ich niektóre rytua³y. Teraz Jim poszed³ na wspó³pracê z Gomezem.  
	AI_Output			(self, other,"Info_Saturas_Fire_Varans_14_06"); //Uda³ siê on wraz z oddzia³em Stra¿y w pobli¿e Nowego Obozu i u¿y³ tajemnego zaklêcia – Przyzwania Ogromnych Ognistych Jaszczurów. 
	AI_Output			(self, other,"Info_Saturas_Fire_Varans_14_07"); //NajwyraŸniej wkrótce chc¹ na nas uderzyæ. 
	AI_Output			(other, self,"Info_Saturas_Fire_Varans_15_08"); //Co takiego?! Istnieje w ogóle takie zaklêcie jak przyzwanie ogromnych ognistych jaszczurów?
	AI_Output			(self, other,"Info_Saturas_Fire_Varans_14_09"); //Ukryte badania Magów Ognia da³y swój plon. Jednak nie spodziewali siê rzecz jasna co stanie siê dalej z ich odkryciem.
	
	AI_Output			(other, self,"Info_Saturas_Fire_Varans_15_10"); //Co bêdzie teraz? Trzeba powstrzymaæ tê przeroœniête gadziny i stra¿ Gomeza.
	AI_Output			(self, other,"Info_Saturas_Fire_Varans_14_11"); //Ty i Myxir udacie siê przed obóz i stoczycie walkê z ognistymi jaszczurami. PóŸniej wezwiesz Najemników by uporaæ siê ze stra¿¹.
	AI_Output			(other, self,"Info_Saturas_Fire_Varans_15_12"); //Pomówiê z Myxirem.
	AI_Output			(self, other,"Info_Saturas_Fire_Varans_14_13"); //S³usznie, œpiesz siê.

    Log_CreateTopic		(OgnisteJaszczury,	LOG_MISSION);
	Log_SetTopicStatus	(OgnisteJaszczury,	LOG_RUNNING);

	B_LogEntry		(OgnisteJaszczury,	"Wezwa³ mnie Saturas. Okazuje siê, ¿e Magnaci przejêli ca³y dorobek Magów Ognia w tym magiczne zaklêcia i runy. Jim poszed³ z nimi na wspó³pracê i u¿y³ tajemnej runy na przyzwanie Ognistych Jaszczurów. Pojawi³y siê te¿ oddzia³y stra¿y Gomeza. Musimy ich powstrzymaæ. Mam pomówiæ z Myxirem by wspólnie pokonaæ ogniste jaszczury a potem razem z Najemnikami mam wybiæ Stra¿ników.");
...
//	SUBMOD
//***************************************************************************
instance DIA_Saturas_Defend_NC (C_INFO)
{
	npc			= KDW_600_Saturas;
	condition	= DIA_Saturas_Defend_NC_Condition;
	information	= DIA_Saturas_Defend_NC_Info;
	permanent	= 0;
	description   = "Ogniste Jaszczury, Cieñ- Magik i...";
	
};

FUNC INT DIA_Saturas_Defend_NC_Condition()
{	
	
	if  (Npc_KnowsInfo(hero, Info_Gorn_NC_AWon)) 
...
FUNC VOID DIA_Saturas_Defend_NC_Info()
{
	AI_Output			(other, self,"DIA_Saturas_Defend_NC_15_01"); //Ogniste Jaszczury, Cieñ- Magik i chroni¹cy go Stra¿nicy nale¿¹ ju¿ do przysz³oœci.
	AI_Output			(self, other,"DIA_Saturas_Defend_NC_14_02"); //To znakomite wieœci. Teraz mo¿emy dalej kontynuowaæ nasze dzia³ania gdy zagro¿enie minê³o.
	AI_Output			(self, other,"DIA_Saturas_Defend_NC_14_03"); //Utalentowany z ciebie Mag. A zarazem ¿o³nierz.
	AI_Output			(self, other,"DIA_Saturas_Defend_NC_14_04"); //Przyjmij tê nagrodê. Zas³u¿y³eœ na ni¹ wykazuj¹c siê umiejêtnoœciami w chwili próby.
	
		    B_GiveInvItems (self,other, ItFo_Potion_Health_Perma_03, 2);
			B_GiveInvItems (self,other, ItFo_Potion_Mana_Perma_03, 2);
	AI_Output			(other, self,"DIA_Saturas_Defend_NC_15_05"); //Dziêkuje.
	AI_Output			(self, other,"DIA_Saturas_Defend_NC_14_06"); //Jeœli bêdê ciê potrzebowa³ to dam znaæ.
	AI_Output			(other, self,"DIA_Saturas_Defend_NC_15_07"); //Oczywiœcie, jestem na twoje wezwanie Saturasie.
	AI_Output			(self, other,"DIA_Saturas_Defend_NC_14_08"); //Dobrze, zatem b¹dŸ czujny.
	B_LogEntry               (OgnisteJaszczury,"Zda³em raport Saturasowi. Pokonaliœmy olbrzymie ogniste jaszczury oraz wspieraj¹cych je Stra¿ników. Wygl¹da na to, ¿e i tym razem plan Magnatów i Gomeza siê nie ziœci.");
    B_GiveXP(500);
};