...
{   
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager 
					...
					Doc_SetMargins	( nDocID, -1, 50, 50, 50, 50, 1   		);  //  0 -> margins are in pixels
					Doc_PrintLine	( nDocID,  0, " "					);
					Doc_PrintLine	( nDocID,  0, "28 wrzeœnia"					);
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_SetFont 	( nDocID, -1, "font_10_book.TGA"	   			); 	// -1 -> all pages 
...
INSTANCE ItQt_IronPack (C_Item)
{
	...
	description			= name;
	TEXT[1]             = "Widnieje na niej napis: Quentin ";
...
{	
	...
	FUNC VOID Use_CraigsMap()
	{
		...
		CreateInvItems (hero, ItMi_CraigsChestKey,1);
...
//-----------------------------------------------------------------------------------1.6.1-----------------------------------------------------------------------------------------
//--------------------------------------------------------------------------
INSTANCE OrcCrystal (C_Item)
{
	name 				=	"Magiczny kryszta³";
...
{	
	...
	scemeName				=	"MAP";	
	on_state[0]				=	UseDouglasMap;

	description				= name;
	TEXT[0]				    = "Wskazuje drogê do ";
	TEXT[1]			        = "skrzyni z siark¹.";

};

	FUNC VOID UseDouglasMap()
	{
		var int nDocID;
		...
		nDocID = 	Doc_CreateMap	()			  ;							// DocManager 
					Doc_SetLevel	( nDocID,	"WORLD.ZEN" );
					Doc_SetPages	( nDocID, 1 );                         
					Doc_SetPage 	( nDocID, 0, "MAP_DOUGLAS.tga", 1	);  //  1 -> DO NOT SCALE 
					Doc_Show		( nDocID 	);
	};
//-----------------------------------------------------------------------------------------------------
INSTANCE ItMi_OrryTowary (C_Item)
{
...
INSTANCE  ItMi_StuartOrcTalisman (C_Item)
{
	name 			= "Orkowy talizman Stuarta";
	...
	on_equip		= Equip_OrcTalismanRR;
	on_unequip		= UnEquip_OrcTalismanRR;
...
FUNC VOID UnEquip_OrcTalismanRR()
{
	Npc_ChangeAttribute(self,ATR_STRENGTH,10);
	self.attribute[ATR_HITPOINTS] = self.attribute[ATR_HITPOINTS] + 30;
		self.attribute[ATR_HITPOINTS_MAX] = self.attribute[ATR_HITPOINTS_MAX] + 30;
...
//
///////////////////////////
INSTANCE ItMi_RichardBlood (C_Item)
{
	name 				=	"Krew Richarda";
	...
	description			= 	name;
	TEXT[0]				= 	"Krew Richarda,";
	TEXT[1]				= 	"muszê j¹ zanieœæ ";
	TEXT[2]				= 	"magom wody.";

};
//-----------------------------------------------------------------------
INSTANCE ItMi_Cronos_Plants (C_Item)
{
	name 				=	"Paczka zió³ Cronosa";
	...
	description			= 	name;
	TEXT[0]				= 	"Paczka ze specjalnymi";
	TEXT[1]				= 	"zio³ami od Virana";
	TEXT[2]				= 	"dla Cronosa.";

};
//-----------------------------------------------------------------------	
INSTANCE ItMi_Netbek_Seed (C_Item)
{
	name 				=	"Worek nasion";
	...
	description			= 	name;
	TEXT[0]				= 	"Nasiona Rombowca Modrego";
	TEXT[1]				= 	"";
	TEXT[2]				= 	"";

};
//-----------------------------------------------------------------------
INSTANCE ItMi_Dp_Antidotum (C_Item)
{
	name 				=	"Antidotum";
	...
	description			= 	name;
	TEXT[0]				= 	"Antidotum dla Richarda.";
};
//-----------------------------------------------------------------------
INSTANCE ItMi_LSZ_BrokenScoop (C_Item)
{
	name 				=	NAME_BrokenScoop;

	mainflag 		=	ITEM_KAT_NONE;
	flags 			=	ITEM_MULTI|ITEM_MISSION;

	visual 				=	"ItMi_BrokenScoop.3DS";
	material 		=	MAT_WOOD;


	description			= 	name;
	TEXT[0]				= 	"Skarb Ziggiego...";
};
//-----------------------------------------------------------------------

INSTANCE ItWr_LSZ_ZiggyToMud (C_Item)
{	
	name 				=	"Wiadomoœæ";
...
{   
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager 
					...
					Doc_SetFont 	( nDocID, -1, "font_10_book.tga"	   			); 	// -1 -> all pages 
					Doc_SetMargins	( nDocID, -1, 50, 50, 50, 50, 1   		);  //  0 -> margins are in pixels
					Doc_PrintLines	( nDocID,  0, "Szanowny I potê¿ny bracie");
					Doc_PrintLine	( nDocID,  0, ""	);
					Doc_PrintLines	( nDocID,  0, "Mi³o mi, ¿e jesteœ wielkim Magnatem I si³¹ Starego obozu. Na pewno trzêsiesz za gard³o ca³¹ Koloni¹ I zarabiasz istne góry rudy. To dla mnie zaszczyt byæ twoim bratem. Teraz chcia³bym ciê odwiedziæ I niech sp³ynie na mnie splendor jaki otrzymujesz z r¹k podw³adnych. Przygotuj dla mnie du¿o jad³a, napoju I kilka piêknych, powabnych towarzyszek. Wkrótce przybêdê by cieszyæ siê razem z tob¹ twym szczêœciem i powodzeniem.");
					Doc_PrintLine	( nDocID,  0, "");
					Doc_PrintLines	( nDocID,  0, "");
					Doc_PrintLines	( nDocID,  0, "Twój brat Ziggy");
					Doc_SetMargins	( nDocID, -1, 200, 50, 50, 50, 1   		);  //  0 -> margins are in pixels (Position des Textes von den Ränder des TGAs aus
...
//-----------------------------------------------------------------------
INSTANCE ItMi_PP_GarpAmulette (C_Item)
{
	name 				=	"Amulet Garpa";
...
//-----------------------------------------------------------------------
	INSTANCE ItMi_PP_PascalOre (C_Item)
{
	name 				=	"Sakiewka Pascala";
	...
	description			= 	name;
	TEXT[0]				= 	"Ruda Garpa dla Pascala.";
};

	
/******************************************************************************************/
INSTANCE ItMi_Worek_Darrion (C_Item)
{
	name 				=	"Ciê¿ka sakiewka";
	...
	scemeName				=	"MAPSEALED";	
	//on_state[0]				=	UseOWorek100;
	description			= name;
	TEXT[2]				= "Bardzo wypchana, ciê¿ka sakiewka Darriona.";
