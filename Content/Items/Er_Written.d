...
{   
	    ...
	    B_LogEntry               (ObitePyski,"Z zapiskÛw Cesara wynika, øe Garp wie coú tej sprawie i moøe poúwiadczyÊ o tym co mia≥o miejsce. Najpierw jednak pogadam z Asghanem. Zg≥oszÍ mu co siÍ wyrabia pod jego nosem.");
};
...
{	
	...
	nDocID = 	Doc_Create		()			  ;								// DocManager 
					...
					Doc_SetMargins	( nDocID, -1, 200, 50, 50, 50, 1   		);  //  0 -> margins are in pixels (Position des Textes von den R‰nder des TGAs aus				
					Doc_Show		( nDocID );
                    B_LogEntry               (Blokada,"Przy dowÛdcy StraønikÛw wchodzπcych w sk≥ad blokady znalaz≥em list. MuszÍ pokazaÊ go Gor Na Tothowi I Cor Angarowi.");
...
{	
	...
	nDocID = 	Doc_Create		()			  ;								// DocManager 
					...
					Doc_SetMargins	( nDocID, -1, 200, 50, 50, 50, 1   		);  //  0 -> margins are in pixels (Position des Textes von den R‰nder des TGAs aus				
					Doc_Show		( nDocID );
                    B_LogEntry               (ZaDlugiJezor,"Norton nas≥a≥ na mnie p≥atnego mordercÍ. Obroni≥em siÍ i znalaz≥em jego list. Zobaczymy co ten draÒ na to powie.");
...
{	
	...
	{   
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager 
					...
					B_LogEntry(HugoBookOrc,"PiszÍ to ja Hugo £owca OrkÛw z grupy Wilsona. PochodzÍ z Klanu Ognia w Nordmarze. BarbarzyÒcy, ktÛrzy tam mieszkajπ nazywani sπ ZabÛjcami OrkÛw. Mamy wiÍc ogromne doúwiadczenie, jeúli chodzi o te bestie.");
