...
INSTANCE ItWr_Map_Przegrupowania(C_Item)
{	
	name 					=	"Mapa";
	...
	scemeName				=	"MAP";	
	on_state[0]				=	UseWorldmap_OrcPO;

	description				= "Mapa orkowego genera³a.";
	TEXT[0]					= "Pokazuje rozmieszczenie";
	TEXT[1]					= "hord orków...";
	
};

	FUNC VOID UseWorldmap_OrcPO()
	{
		var int nDocID;
		...
		nDocID = 	Doc_CreateMap	()			  ;							// DocManager 
					Doc_SetLevel	( nDocID,	"WORLD.ZEN" );
					Doc_SetPages	( nDocID, 1 );                         
					Doc_SetPage 	( nDocID, 0, "MAP_PRZEGRUPOWANIA.tga", 	1	);  //  1 -> DO NOT SCALE 
					Doc_Show		( nDocID 	);
	};
...
/****************************************************
******* PIERŒCIEÑ SZAMANA Z WIEZY MGIE£
******* 
******* 
*****************************************************/
	instance ITRI_GWM(C_Item)
{
	name 					=	"Pierœcieñ";
	...
	visual 					=	"ITMI_RING_GWM.3DS";

	//visual_skin 				=	0;
	material 				=	MAT_METAL;
	on_equip				=	Equip_ITRI_GWM;
	on_unequip				=	UnEquip_ITRI_GWM;

	description		= "Pierœcieñ";

};

	FUNC VOID Equip_ITRI_GWM()
	{
	Snd_Play 	    ("MFX_DESTROYUNDEAD_CAST" );
	Wld_PlayEffect  ("spellFX_MassDeath",hero,hero, 0, 0, 0, TRUE);
...
INSTANCE ItMi_Cloth_BG (C_Item)
{
	...
	visual 				=	"ItMi_Cloth.3ds";
	material 			=	MAT_CLAY;

	description			= 	name;
	TEXT[1]				= 	"Strasznie cuchnie";
	TEXT[2	]				= 	"alkoholem.";

};

INSTANCE ItMi_Items_BG (C_Item)
{
...
******* 
*****************************************************/
INSTANCE ItMi_Throphy_PartyMembers (C_Item)
{
	name 				=	"Trofea od Alexa";
	...
	description			= 	name;
	TEXT[1]				= 	"Nagroda dla reszty";
	TEXT[2]				= 	"za pomoc ze stra¿nikami.";

};

INSTANCE ItMi_Throphy_ZM (C_Item)
{
	name 				=	"Trofea od Alexa";
    ...
    scemeName	        =	"MAPSEALED";
	on_state[0]	        =	Use_ItMi_Throphy_ZM;
	...
	description			= 	name;
	TEXT[1]				= 	"Moja nagroda za pomoc";
	TEXT[2]				= 	"ze stra¿nikami...";

};
FUNC VOID Use_ItMi_Throphy_ZM()
{
B_PlayerFindItem (ItAt_Shadow_02,1);              
B_PlayerFindItem (ItAt_Lurker_01,1);              
B_PlayerFindItem (ItAt_Waran_Skin,1);              
B_PlayerFindItem (ItAt_Snapper_Skin,1);              
B_PlayerFindItem (ItAt_Teeth_01,8);              
B_PlayerFindItem (ItAt_Crawler_02,4);              
};


/******************************************************************************************/
INSTANCE ItMi_Steve_Nugget (C_Item)
{
	name 				=	"Wypchana sakiewka";
	...
	description			= name;
	TEXT[2]				= "Bardzo wypchana, ciê¿ka sakiewka.";
	TEXT[4]				= NAME_Value;					COUNT[4]	= value;
};